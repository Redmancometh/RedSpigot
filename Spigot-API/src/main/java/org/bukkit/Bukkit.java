...
public final class Bukkit {
	...
	public static void setServer(Server server) {
		/*
		 * if (Bukkit.server != null) { throw new
		 * UnsupportedOperationException("Cannot redefine singleton Server"); }
		 */
		Bukkit.server = server;
		server.getLogger()
				.info("This server is running " + getName() + " version " + getVersion() + " (Implementing API version " + getBukkitVersion() + ")");
	}
	 ...
	 * {@linkplain Collections#unmodifiableCollection(Collection) view} is a
	 * reused object, making some operations like {@link Collection#size()}
	 * zero-allocation.
	 * <p>
	 * The collection is a view backed by the internal representation, such
	 * that, changes to the internal state of the server will be reflected
	 * immediately. However, the reuse of the returned collection (identity) is
	 * not strictly guaranteed for future or all implementations. Casting the
	 * collection, or relying on interface implementations (like
	 * {@link Serializable} or {@link List}), is deprecated.
	 * <p>
	 * Iteration behavior is undefined outside of self-contained main-thread
	 * uses. Normal and immediate iterator use without consequences that affect
	 * the collection are fully supported. The effects following
	 * (non-exhaustive) {@link Entity#teleport(Location) teleportation},
	 * {@link Player#setHealth(double) death}, and
	 * {@link Player#kickPlayer( String) kicking} are undefined. Any use of this
	 * collection from asynchronous threads is unsafe.
	 * <p>
	...
	public static Logger getLogger() {
		// System.out.println("SERVER NULL: " + (server == null));
	 ...
	 * @param permission
	 *            the required permission {@link Permissible permissibles} must
	 *            have to receive the broadcast
	 ...
	 * <p>
	 * Bans by name are no longer supported and this method will return null
	 * when trying to request them. The replacement is bans by UUID.
	 *
	 * @param type
	 ...
	 * 
	 * @param world
	 *            the world to create the ChunkData for
	 ...
	 * @param flags
	 *            an optional list of flags to set on the boss bar
