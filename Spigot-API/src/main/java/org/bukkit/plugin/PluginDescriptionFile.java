...
public final class PluginDescriptionFile {
			...
			return new Yaml(new SafeConstructor() {
				{
					yamlConstructors.put(null, new AbstractConstruct() {
						...
						public Object construct(final Node node) {
							...
							// Unknown awareness - provide a graceful
							// substitution
	 ...
	 * period (a-z,A-Z,0-9, _.-). Any other character will cause the plugin.yml
	 * to fail loading.
	 * <li>Used to determine the name of the plugin's data folder. Data folders
	 * are placed in the ./plugins/ directory by default, but this behavior
	 * should not be relied on. {@link Plugin#getDataFolder()} should be used to
	 ...
	 * <a href="http://yaml.org/spec/current.html#id2503232">surround the
	 * message with double-quote</a>: <blockquote>
	 * 
	 * <pre>
	 ...
	 * <a href="http://yaml.org/spec/current.html#id2502325">nested
	 * mappings</a>. The primary (top-level, no intendentation) node is
	 * `<code>commands</code>', while each individual command name is indented,
	 * indicating it maps to some value (in our case, the properties of the
	 * table above).
	 * <p>
	 ...
	 * <tr>
	 * <td><code>description</code></td>
	 * <td>Plaintext (user-friendly) description of what the permission is
	 * for.</td>
	 * <td><blockquote>
	 * 
	 * <pre>
	 ...
	 * <a href="http://en.wikipedia.org/wiki/YAML#Lists">list</a> of names.
	 * Using a list will treat all children associated positively to their
	 * parent.
		...
		if (permissions == null) {
			if (lazyPermissions == null) {
				...
				permissions = ImmutableList.copyOf(Permission.loadPermissions(lazyPermissions,
						"Permission node '%s' in plugin description file for " + getFullName() + " is invalid", defaultPerm));
	 ...
	 * is compiled. Some implementions may define extra awarenesses that are not
	 * included in the API. Any unrecognized awareness (one unsupported or in a
	 * future version) will cause a dummy object to be created instead of
	 * failing.
	 * 
	 * <ul>
	 ...
	 * <li>Unrecognized (future / unimplemented) entries are quietly replaced by
	 * a generic object that implements PluginAwareness.
	 * <li>A type of awareness must be defined by the runtime and acknowledged
	 * by the API, effectively discluding any derived type from any plugin's
	 * classpath.
