	...
	public void executeSpigotTimings(CommandSender sender, String[] args) {
		...
		if ("reset".equals(args[0])) {
			...
			try {
				...
				// TODON: Json saving here.
				// fileTimings.println(
				// Bukkit.spigot().getConfig().saveToString() );
				fileTimings.println("</spigotConfig>");
					...
					for (RegisteredListener listener : HandlerList.getRegisteredListeners(plugin)) {
						if (listener instanceof TimedRegisteredListener) {
							...
							if (count > 0 && eventClass != null) {
								fileTimings.println("    " + eventClass.getSimpleName() + (trl.hasMultiple() ? " (and sub-classes)" : "") + " Time: "
										+ time + " Count: " + count + " Avg: " + avg);
							}
						}
					}
	...
	private static class PasteThread extends Thread {
		...
		public void run() {
			try {
				...
				com.google.gson.JsonObject location = new com.google.gson.Gson().fromJson(new java.io.InputStreamReader(con.getInputStream()),
