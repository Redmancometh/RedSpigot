...
public abstract class EntityArrow extends Entity implements IProjectile {
	...
	public void B_() {
		...
		if (this.inGround) {
			...
			if ((block != this.av || i != this.aw) && !this.world.a(this.getBoundingBox().g(0.05D))) {
				...
				++this.ax;
				if (this.ax >= world.spigotConfig.arrowDespawnRate) { // Spigot
																		// -
																		// First
																		// int
																		// after
			...
			if (this.isCritical()) {
				for (int j = 0; j < 4; ++j) {
					this.world.addParticle(EnumParticle.CRIT, this.locX + this.motX * (double) j / 4.0D, this.locY + this.motY * (double) j / 4.0D,
							this.locZ + this.motZ * (double) j / 4.0D, -this.motX, -this.motY + 0.2D, -this.motZ, new int[0]);
				}
			}
			...
			for (this.pitch = (float) (MathHelper.c(this.motY, (double) f1) * 57.2957763671875D); this.pitch
			...
			if (this.isInWater()) {
				for (int k = 0; k < 4; ++k) {
					...
					this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX - this.motX * 0.25D, this.locY - this.motY * 0.25D,
							this.locZ - this.motZ * 0.25D, this.motX, this.motY, this.motZ, new int[0]);
				}
			...
			if (entity.damageEntity(damagesource, (float) i)) {
				if (entity instanceof EntityLiving) {
					...
					if (this.knockbackStrength > 0) {
						...
						if (f1 > 0.0F) {
							entityliving.f(this.motX * (double) this.knockbackStrength * 0.6000000238418579D / (double) f1, 0.1D,
									this.motZ * (double) this.knockbackStrength * 0.6000000238418579D / (double) f1);
						}
					}
					...
					this.a(entityliving);
					if (this.shooter != null && entityliving != this.shooter && entityliving instanceof EntityHuman
	...
	public void b(NBTTagCompound nbttagcompound) {
		...
		MinecraftKey minecraftkey = (MinecraftKey) blockRegistry.b(this.av);
	...
	public void a(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKeyOfType("inTile", 8)) {
			this.av = blockRegistry.getByName(nbttagcompound.getString("inTile"));
		} else {
			this.av = blockRegistry.getById(nbttagcompound.getByte("inTile") & 255);
		}
	...
	public void d(EntityHuman entityhuman) {
		if (!this.world.isClientSide && this.inGround && this.shake <= 0) {
			...
			if (this.fromPlayer == PickupStatus.ALLOWED && entityhuman.inventory.canHold(itemstack) > 0) {
				PlayerPickupArrowEvent event = new PlayerPickupArrowEvent((org.bukkit.entity.Player) entityhuman.getBukkitEntity(),
						new org.bukkit.craftbukkit.entity.CraftItem(this.world.getServer(), this, item),
						(org.bukkit.entity.Arrow) this.getBukkitEntity());
			...
			boolean flag = this.fromPlayer == EntityArrow.PickupStatus.ALLOWED
					|| this.fromPlayer == EntityArrow.PickupStatus.CREATIVE_ONLY && entityhuman.abilities.canInstantlyBuild;
