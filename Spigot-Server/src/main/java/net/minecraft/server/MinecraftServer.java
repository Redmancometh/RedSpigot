...
public abstract class MinecraftServer implements ICommandListener, Runnable, IAsyncTaskHandler, IMojangStatistics {
	...
	@Autowired
	private YggdrasilAuthenticationService V;
	@Autowired
	private MinecraftSessionService W;
	@Autowired
	private GameProfileRepository X;
	@Autowired
	private UserCache Y;
	@Autowired
	private ServerPing q;
	@Autowired
	public MethodProfiler methodProfiler;
	@Autowired
	@Qualifier("tickable-list")
	private List<ITickable> o;
	@Autowired
	...
	@Autowired
	private DataConverterManager dataconvertermanager;
	@Autowired
	...
	public MinecraftServer() {
		...
		this.serverThread = primaryThread = new Thread(this, "Server thread"); // Moved
																				// from
																				// main
	}
	...
	public void a(String s, String s1, long i, WorldType worldtype, String s2) {
		...
		for (int j = 0; j < worldCount; ++j) {
			...
			if (j == 0) {
				...
				worlddata.checkName(s1); // CraftBukkit - Migration did not
											// rewrite the level.dat; This
											// forces 1.8 to take the last
											// loaded world as respawn (in this
											// case the end)
				if (this.V()) {
					...
					if (newWorld.exists()) {
						...
						if (oldWorld.renameTo(newWorld)) {
							MinecraftServer.LOGGER
									.info("Success! To restore " + worldType + " in the future, simply move " + newWorld + " to " + oldWorld);
				...
				// world =, b0 to dimension, s1 to name, added Environment and
				// gen
				WorldData worlddata = idatamanager.getWorldData();
				if (worlddata == null) {
					worlddata = new WorldData(worldsettings, name);
				}
				worlddata.checkName(name); // CraftBukkit - Migration did not
											// rewrite the level.dat; This
											// forces 1.8 to take the last
											// loaded world as respawn (in this
											// case the end)
			...
			world.addIWorldAccess(new RedWorldManager(this, world));
			if (!this.R()) {
	...
	protected void l() {
		...
		// CraftBukkit start - fire WorldLoadEvent and handle whether or not to
		// keep the spawn in memory
			...
			if (worldserver != null) {
				if (!flag) {
					MinecraftServer.LOGGER.info("Saving chunks for level \'{}\'/{}", worldserver.getWorldData().getName(),
							worldserver.worldProvider.getDimensionManager().b());
				}
	...
	public void stop() throws ExceptionWorldConflict { // CraftBukkit - added
														// throws
		...
		if (this.v != null) {
			...
			} // CraftBukkit - SPIGOT-625 - give server at least a chance to
				// send packets
		}
	...
	public void run() {
		try {
			...
			this.q.setServerInfo(new ServerPing.ServerData(serverVersion, 340));
			this.a(this.q);
			// Spigot start
	...
	protected void C() throws ExceptionWorldConflict { // CraftBukkit - added
														// throws
		...
		if (autosavePeriod > 0 && this.ticks % autosavePeriod == 0) { // CraftBukkit
			...
			this.v.savePlayers();
			// Spigot Start
			// We replace this with saving each individual world as
			// this.saveChunks(...) is broken,
			// and causes the main thread to sleep for random amounts of time
	...
	public void D() {
		...
		// Send time updates to everyone, it will get the right time from the
		// world the player is in.
		if (this.ticks % 20 == 0) {
			for (int i = 0; i < this.getPlayerList().players.size(); ++i) {
				EntityPlayer entityplayer = (EntityPlayer) this.getPlayerList().players.get(i);
				entityplayer.playerConnection.sendPacket(new PacketPlayOutUpdateTime(entityplayer.world.getTime(), entityplayer.getPlayerTime(),
		...
		for (i = 0; i < this.worlds.size(); ++i) { // CraftBukkit
			 ...
			 * PacketPlayOutUpdateTime(worldserver.getTime(),
	...
	public void F() {
		/*
		 * CraftBukkit start - prevent abuse this.serverThread = new
		 * Thread(this, "Server thread"); this.serverThread.start(); //
		 * CraftBukkit end
		 */
	}
	...
	public List<String> tabCompleteCommand(ICommandListener icommandlistener, String s, @Nullable BlockPosition blockposition, boolean flag) {
		 ...
		 * 
		 * return arraylist; } else { boolean flag2 = !s.contains(" "); List
		 * list = this.b.a(icommandlistener, s, blockposition);
		 * 
		 * if (!list.isEmpty()) { Iterator iterator = list.iterator();
		 * 
		 * while (iterator.hasNext()) { String s3 = (String) iterator.next();
		 * 
		 * if (flag2 && !flag) { arraylist.add("/" + s3); } else {
		 * arraylist.add(s3); } } }
		 * 
		 * return arraylist; }
		 */
			...
			for (int j = 0; j < this.worlds.size(); ++j) {
				...
				if (worldserver != null) {
					...
					mojangstatisticsgenerator.a("world[" + i + "][dimension]",
							Integer.valueOf(worldserver.worldProvider.getDimensionManager().getDimensionID()));
	...
	/**
	 * Don't ever call this from anywhere ever. This shit is autowired and the
	 * server logic will break down completely.
	 * 
	 * @param playerlist
	 */
	...
	public void reload() {
		Bukkit.getLogger().severe("Reloads don't turn out well, and have been removed. Command ignored");
	}
