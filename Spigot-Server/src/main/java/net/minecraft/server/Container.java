...
public abstract class Container {
	...
	public void b() {
		for (int i = 0; i < this.slots.size(); ++i) {
			...
			if (!ItemStack.fastMatches(itemstack1, itemstack)
					|| (tickCount % cfg.getItemDirtyTicks() == 0 && !ItemStack.matches(itemstack1, itemstack))) { // Spigot
	...
	public ItemStack a(int i, int j, InventoryClickType inventoryclicktype, EntityHuman entityhuman) {
		...
		if (inventoryclicktype == InventoryClickType.QUICK_CRAFT) {
			...
			if ((i1 != 1 || this.g != 2) && i1 != this.g) {
				...
				if (slot != null && a(slot, itemstack1, true) && slot.isAllowed(itemstack1)
						&& (this.dragType == 2 || itemstack1.getCount() > this.h.size()) && this.b(slot)) {
					this.h.add(slot);
				}
			} else if (this.g == 2) {
				if (!this.h.isEmpty()) {
					...
					while (iterator.hasNext()) {
						...
						if (slot1 != null && a(slot1, itemstack3, true) && slot1.isAllowed(itemstack3)
								&& (this.dragType == 2 || itemstack3.getCount() >= this.h.size()) && this.b(slot1)) {
							...
							draggedSlots.put(slot1.rawSlotIndex, itemstack4); // CraftBukkit
																				// -
																				// Put
																				// in
																				// map
					...
					// It's essential that we set the cursor to the new value
					// here to prevent item duplication if a plugin closes the
					// inventory.
					ItemStack oldCursor = playerinventory.getCarried();
					playerinventory.setCarried(CraftItemStack.asNMSCopy(newcursor));

					InventoryDragEvent event = new InventoryDragEvent(view, (newcursor.getType() != org.bukkit.Material.AIR ? newcursor : null),
							CraftItemStack.asBukkitCopy(oldCursor), this.dragType == 1, eventmap);
					entityhuman.world.getServer().getPluginManager().callEvent(event);

					// Whether or not a change was made to the inventory that
					// requires an update.
					...
					if (event.getResult() != Result.DENY) {
						...
						// The only time the carried item will be set to null is
						// if the inventory is closed by the server.
						// If the inventory is closed by the server, then the
						// cursor items are dropped. This is why we change the
						// cursor early.
			...
			if ((inventoryclicktype == InventoryClickType.PICKUP || inventoryclicktype == InventoryClickType.QUICK_MOVE) && (j == 0 || j == 1)) {
				if (i == -999) {
					...
					for (itemstack2 = this.shiftClick(entityhuman, i); !itemstack2.isEmpty()
							&& ItemStack.c(slot2.getItem(), itemstack2); itemstack2 = this.shiftClick(entityhuman, i)) {
					...
					if (slot2 != null) {
						...
						if (itemstack2.isEmpty()) {
							...
							if (itemstack1.isEmpty()) {
								...
								if (itemstack2.getItem() == itemstack1.getItem() && itemstack2.getData() == itemstack1.getData()
										&& ItemStack.equals(itemstack2, itemstack1)) {
									k1 = j == 0 ? itemstack1.getCount() : 1;
							...
							} else if (itemstack2.getItem() == itemstack1.getItem() && itemstack1.getMaxStackSize() > 1
									&& (!itemstack2.usesData() || itemstack2.getData() == itemstack1.getData())
									&& ItemStack.equals(itemstack2, itemstack1) && !itemstack2.isEmpty()) {
						...
						slot2.f();
						// CraftBukkit start - Make sure the client has the
						// right slot contents
						if (entityhuman instanceof EntityPlayer && slot2.getMaxStackSize() != 64) {
							((EntityPlayer) entityhuman).playerConnection
									.sendPacket(new PacketPlayOutSetSlot(this.windowId, slot2.rawSlotIndex, slot2.getItem()));
							// Updating a crafting inventory makes the client
							// reset the result slot, have to send it again
							if (this.getBukkitView().getType() == InventoryType.WORKBENCH
									|| this.getBukkitView().getType() == InventoryType.CRAFTING) {
								((EntityPlayer) entityhuman).playerConnection
										.sendPacket(new PacketPlayOutSetSlot(this.windowId, 0, this.getSlot(0).getItem()));
							}
						}
						// CraftBukkit end
					}
			...
			} else if (inventoryclicktype == InventoryClickType.CLONE && entityhuman.abilities.canInstantlyBuild
					&& playerinventory.getCarried().isEmpty() && i >= 0) {
				slot2 = (Slot) this.slots.get(i);
	...
	protected boolean a(ItemStack itemstack, int i, int j, boolean flag) {
		...
		if (itemstack.isStackable()) {
			while (!itemstack.isEmpty()) {
				...
				if (!itemstack1.isEmpty() && itemstack1.getItem() == itemstack.getItem()
						&& (!itemstack.usesData() || itemstack.getData() == itemstack1.getData()) && ItemStack.equals(itemstack, itemstack1)) {
	...
	public static boolean a(@Nullable Slot slot, ItemStack itemstack, boolean flag) {
		...
		return !flag1 && itemstack.doMaterialsMatch(slot.getItem()) && ItemStack.equals(slot.getItem(), itemstack)
				? slot.getItem().getCount() + (flag ? 0 : itemstack.getCount()) <= itemstack.getMaxStackSize()
				: flag1;
	}
