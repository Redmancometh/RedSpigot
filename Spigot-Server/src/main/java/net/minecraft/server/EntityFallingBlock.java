...
public class EntityFallingBlock extends Entity {
	...
	public void B_() {
		...
		if (this.block.getMaterial() == Material.AIR) {
			...
			if (!this.world.isClientSide) {
				...
				if (flag && d0 > 1.0D) {
					MovingObjectPosition movingobjectposition = this.world.rayTrace(new Vec3D(this.lastX, this.lastY, this.lastZ),
							new Vec3D(this.locX, this.locY, this.locZ), true);
				...
				if (!this.onGround && !flag1) {
					if (this.ticksLived > 100 && !this.world.isClientSide && (blockposition.getY() < 1 || blockposition.getY() > 256)
							|| this.ticksLived > 600) {
					...
					if (iblockdata.getBlock() != blocks.PISTON_EXTENSION) {
						...
						if (!this.f) {
							...
							if (this.world.a(block, blockposition, true, EnumDirection.UP, (Entity) null)
									&& (flag1 || !BlockFalling.x(this.world.getType(blockposition.down())))) {
								if (CraftEventFactory.callEntityChangeBlockEvent(this, blockposition, this.block.getBlock(),
	...
	protected void b(NBTTagCompound nbttagcompound) {
		...
		MinecraftKey minecraftkey = (MinecraftKey) blockRegistry.b(block);
	...
	protected void a(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKeyOfType("Block", 8)) {
			this.block = blockRegistry.getByName(nbttagcompound.getString("Block")).fromLegacyData(i);
		} else if (nbttagcompound.hasKeyOfType("TileID", 99)) {
			this.block = blockRegistry.getById(nbttagcompound.getInt("TileID")).fromLegacyData(i);
		} else {
			this.block = blockRegistry.getById(nbttagcompound.getByte("Tile") & 255).fromLegacyData(i);
		}
	...
	public void appendEntityCrashDetails(CrashReportSystemDetails crashreportsystemdetails) {
		...
		if (this.block != null) {
			...
			crashreportsystemdetails.a("Immitating block ID", (Object) Integer.valueOf(blockRegistry.getId(block)));
