...
public abstract class World implements IBlockAccess {
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider, MethodProfiler methodprofiler, boolean flag,
			ChunkGenerator gen, org.bukkit.World.Environment env) {
		...
		getWorldBorder().a(new IWorldBorderListener() {
			public void a(WorldBorder worldborder, double d0) {
				getServer().getHandle().sendAll(new PacketPlayOutWorldBorder(worldborder, PacketPlayOutWorldBorder.EnumWorldBorderAction.SET_SIZE),
						worldborder.world);
			}

			public void a(WorldBorder worldborder, double d0, double d1, long i) {
				getServer().getHandle().sendAll(new PacketPlayOutWorldBorder(worldborder, PacketPlayOutWorldBorder.EnumWorldBorderAction.LERP_SIZE),
						worldborder.world);
			}

			public void a(WorldBorder worldborder, double d0, double d1) {
				getServer().getHandle().sendAll(new PacketPlayOutWorldBorder(worldborder, PacketPlayOutWorldBorder.EnumWorldBorderAction.SET_CENTER),
						worldborder.world);
			}

			public void a(WorldBorder worldborder, int i) {
				getServer().getHandle().sendAll(
						new PacketPlayOutWorldBorder(worldborder, PacketPlayOutWorldBorder.EnumWorldBorderAction.SET_WARNING_TIME),
						worldborder.world);
			}

			public void b(WorldBorder worldborder, int i) {
				getServer().getHandle().sendAll(
						new PacketPlayOutWorldBorder(worldborder, PacketPlayOutWorldBorder.EnumWorldBorderAction.SET_WARNING_BLOCKS),
						worldborder.world);
			}
		...
		// CraftBukkit end
		timings = new SpigotTimings.WorldTimingsHandler(this); // Spigot - code
																// below can
																// generate new
																// world and
																// access
	...
	public IBlockData c(BlockPosition blockposition) {
		...
		for (blockposition1 = new BlockPosition(blockposition.getX(), this.getSeaLevel(), blockposition.getZ()); !this
	...
	private boolean isValidLocation(BlockPosition blockposition) {
		return !this.E(blockposition) && blockposition.getX() >= -30000000 && blockposition.getZ() >= -30000000 && blockposition.getX() < 30000000
	...
	public boolean areChunksLoaded(BlockPosition blockposition, int i, boolean flag) {
		return this.isAreaLoaded(blockposition.getX() - i, blockposition.getY() - i, blockposition.getZ() - i, blockposition.getX() + i,
				blockposition.getY() + i, blockposition.getZ() + i, flag);
	}
	...
	public boolean areChunksLoadedBetween(BlockPosition blockposition, BlockPosition blockposition1, boolean flag) {
		return this.isAreaLoaded(blockposition.getX(), blockposition.getY(), blockposition.getZ(), blockposition1.getX(), blockposition1.getY(),
				blockposition1.getZ(), flag);
	}
	...
	public boolean b(StructureBoundingBox structureboundingbox, boolean flag) {
		return this.isAreaLoaded(structureboundingbox.a, structureboundingbox.b, structureboundingbox.c, structureboundingbox.d,
				structureboundingbox.e, structureboundingbox.f, flag);
	}
	...
	public boolean setTypeAndData(BlockPosition blockposition, IBlockData iblockdata, int i) {
		...
		if (this.captureTreeGeneration) {
			...
			if (blockstate == null) {
				blockstate = org.bukkit.craftbukkit.block.CraftBlockState.getBlockState(this, blockposition.getX(), blockposition.getY(),
						blockposition.getZ(), i);
			}
		...
		if (this.E(blockposition)) {
			...
			if (iblockdata1 == null) {
				...
				/*
				 * if ((i & 2) != 0 && (!this.isClientSide || (i & 4) == 0) &&
				 * chunk.isReady()) { this.notify(blockposition, iblockdata1,
				 * iblockdata, i); }
				 * 
				 * if (!this.isClientSide && (i & 1) != 0) {
				 * this.update(blockposition, iblockdata1.getBlock(), true); if
				 * (iblockdata.n()) {
				...
				// CraftBukkit start
				if (!this.captureBlockStates) { // Don't notify clients or
												// update physics while
												// capturing blockstates
	...
	// CraftBukkit start - Split off from above in order to directly send client
	// and physic updates
	public void notifyAndUpdatePhysics(BlockPosition blockposition, Chunk chunk, IBlockData oldBlock, IBlockData newBlock, int i) {
		if ((i & 2) != 0 && (!this.isClientSide || (i & 4) == 0) && (chunk == null || chunk.isReady())) { // allow
																											// chunk
																											// to
																											// be
																											// null
			...
			try {
				...
				if (world != null) {
					BlockPhysicsEvent event = new BlockPhysicsEvent(
							world.getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ()), CraftMagicNumbers.getId(block));
				...
				while (k1-- >= 0) {
					...
					if (!flag1 || iblockdata1.getMaterial() == Material.PORTAL || iblockdata1.d(this, blockposition) != Block.k) {
						if (block1.a(iblockdata1, flag)) {
							...
							movingobjectposition1 = new MovingObjectPosition(MovingObjectPosition.EnumMovingObjectType.MISS, vec3d, enumdirection,
									blockposition);
						}
					}
				}
	...
	public void a(@Nullable EntityHuman entityhuman, BlockPosition blockposition, SoundEffect soundeffect, SoundCategory soundcategory, float f,
			float f1) {
		this.a(entityhuman, (double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D, (double) blockposition.getZ() + 0.5D,
				soundeffect, soundcategory, f, f1);
	}

	public void a(@Nullable EntityHuman entityhuman, double d0, double d1, double d2, SoundEffect soundeffect, SoundCategory soundcategory, float f,
			float f1) {
		for (int i = 0; i < this.u.size(); ++i) {
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { // Changed
																		// signature,
																		// added
																		// SpawnReason
		...
		if (entity instanceof EntityLiving && !(entity instanceof EntityPlayer)) {
			...
			// Not all projectiles extend EntityProjectile, so check for Bukkit
			// interface instead
	...
	public void removeEntity(Entity entity) {
		...
		if (!guardEntityList) { // Spigot - It will get removed after the tick
								// if we are ticking
			int i = entity.ab;
			int j = entity.ad;
	...
	public boolean a(AxisAlignedBB axisalignedbb) {
		return this.a((Entity) null, axisalignedbb, true, Lists.<AxisAlignedBB>newArrayList()); // CraftBukkit
																								// -
																								// decompile
																								// error
	}
	...
	public BlockPosition q(BlockPosition blockposition) {
		...
		for (blockposition1 = new BlockPosition(blockposition.getX(), chunk.g() + 16, blockposition.getZ()); blockposition1
				.getY() >= 0; blockposition1 = blockposition2) {
	...
	public void tickEntities() {
		...
		if (tickPosition < 0)
			tickPosition = 0;
		for (entityLimiter.initTick(); entitiesThisCycle < entityList.size()
				&& (entitiesThisCycle % 10 != 0 || entityLimiter.shouldContinue()); tickPosition++, entitiesThisCycle++) {
		...
		for (tileLimiter.initTick(); tilesThisCycle < tileEntityListTick.size()
				&& (tilesThisCycle % 10 != 0 || tileLimiter.shouldContinue()); tileTickPosition++, tilesThisCycle++) {
		...
		if (!this.b.isEmpty()) {
			for (int i1 = 0; i1 < this.b.size(); ++i1) {
				...
				if (!tileentity1.y()) {
					 ...
					 * (!this.tileEntityList.contains(tileentity1)) {
	...
	public void entityJoinedWorld(Entity entity, boolean flag) {
		...
		// CraftBukkit start - check if chunks are loaded as done in previous
		// versions
		// TODO: Go back to Vanilla behaviour when comfortable
		// Spigot start
		// Chunk startingChunk =
		// this.getChunkIfLoaded(MathHelper.floor(entity.locX) >> 4,
		// MathHelper.floor(entity.locZ) >> 4);
		...
		if (f >= 0.15F) {
			...
			if (blockposition.getY() >= 0 && blockposition.getY() < 256 && this.getBrightness(EnumSkyBlock.BLOCK, blockposition) < 10) {
				...
				if ((block == blocks.WATER || block == blocks.FLOWING_WATER) && ((Integer) iblockdata.get(BlockFluids.LEVEL)).intValue() == 0) {
					...
					boolean flag1 = this.G(blockposition.west()) && this.G(blockposition.east()) && this.G(blockposition.north())
							&& this.G(blockposition.south());
	...
	public boolean c(EnumSkyBlock enumskyblock, BlockPosition blockposition) {
		...
		if (chunk == null || !chunk.areNeighborsLoaded(
				1) /* !this.areChunksLoaded(blockposition, 17, false) */) {
			// CraftBukkit end
			return false;
		} else {
			int i = 0;
	...
	public <T extends Entity> List<T> b(Class<? extends T> oclass, Predicate<? super T> predicate) {
		...
		while (iterator.hasNext()) {
			...
			if (oclass.isAssignableFrom(entity.getClass()) && predicate.apply((T) entity)) { // CraftBukkit
																								// -
																								// fix
																								// decompile
																								// error
	...
	public int a(Class<?> oclass) {
		...
		while (iterator.hasNext()) {
			Entity entity = (Entity) iterator.next();
			// CraftBukkit start - Split out persistent check, don't apply it to
			// special persistent mobs
			...
			if (oclass.isAssignableFrom(entity.getClass())) {
				// if ((!(entity instanceof EntityInsentient) ||
				// !((EntityInsentient) entity).isPersistent()) &&
				// oclass.isAssignableFrom(entity.getClass())) {
				// CraftBukkit end
				++i;
			}
		}
	...
	public boolean a(Block block, BlockPosition blockposition, boolean flag, EnumDirection enumdirection, @Nullable Entity entity) {
		...
		BlockCanBuildEvent event = new BlockCanBuildEvent(
				this.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ()), CraftMagicNumbers.getId(block),
				defaultReturn);
		this.getServer().getPluginManager().callEvent(event);
	...
	public EntityHuman a(BlockPosition blockposition, double d0, double d1) {
		return this.a((double) ((float) blockposition.getX() + 0.5F), (double) ((float) blockposition.getY() + 0.5F),
				(double) ((float) blockposition.getZ() + 0.5F), d0, d1, (Function) null, (Predicate) null);
	}

	@Nullable
	public EntityHuman a(double d0, double d1, double d2, double d3, double d4, @Nullable Function<EntityHuman, Double> function,
		...
		for (int i = 0; i < this.players.size(); ++i) {
			...
			if (!entityhuman1.abilities.isInvulnerable && entityhuman1.isAlive() && !entityhuman1.isSpectator()
					&& (predicate == null || predicate.apply(entityhuman1))) {
	...
	public boolean e(int i, int j) {
		...
		return k >= -128 && k <= 128 && l >= -128 && l <= 128 && this.keepSpawnInMemory; // CraftBukkit
																							// -
																							// Added
