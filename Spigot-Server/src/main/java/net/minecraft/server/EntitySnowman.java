...
public class EntitySnowman extends EntityGolem implements IRangedEntity {
	...
	public void n() {
		...
		if (!this.world.isClientSide) {
			...
			if (this.world.getBiome(new BlockPosition(i, 0, k)).a(new BlockPosition(i, j, k)) > 1.0F) {
				this.damageEntity(CraftEventFactory.MELTING, 1.0F); // CraftBukkit
																	// -
																	// DamageSource.BURN
																	// ->
	...
	protected boolean a(EntityHuman entityhuman, EnumHand enumhand) {
		...
		if (itemstack.getItem() == Items.SHEARS && this.hasPumpkin() && !this.world.isClientSide) {
			// CraftBukkit start
			PlayerShearEntityEvent event = new PlayerShearEntityEvent((org.bukkit.entity.Player) entityhuman.getBukkitEntity(),
