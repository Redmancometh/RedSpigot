...
public class BlockPumpkin extends BlockFacingHorizontal {
	...
	private final Predicate<IBlockData> e = new Predicate() {
	...
	private void c(World world, BlockPosition blockposition) {
		...
		BlockStateListPopulator blockList = new BlockStateListPopulator(world.getWorld()); // CraftBukkit
																							// -
																							// Use
		...
		if (shapedetector_shapedetectorcollection != null) {
			...
			entitysnowman.setPositionRotation((double) blockposition1.getX() + 0.5D, (double) blockposition1.getY() + 0.05D,
					(double) blockposition1.getZ() + 0.5D, 0.0F, 0.0F);
			// CraftBukkit start
			if (world.addEntity(entitysnowman, SpawnReason.BUILD_SNOWMAN)) {
				...
				for (j = 0; j < 120; ++j) {
					world.addParticle(EnumParticle.SNOW_SHOVEL, (double) blockposition1.getX() + world.random.nextDouble(),
			...
			if (shapedetector_shapedetectorcollection != null) {
				for (i = 0; i < this.getDetectorIronGolem().c(); ++i) {
					for (int k = 0; k < this.getDetectorIronGolem().b(); ++k) {
						// CraftBukkit start
						// world.setTypeAndData(shapedetector_shapedetectorcollection.a(i,
						// k, 0).getPosition(), blocks.AIR.getBlockData(), 2);
				...
				entityirongolem.setPositionRotation((double) blockposition2.getX() + 0.5D, (double) blockposition2.getY() + 0.05D,
						(double) blockposition2.getZ() + 0.5D, 0.0F, 0.0F);
				// CraftBukkit start
