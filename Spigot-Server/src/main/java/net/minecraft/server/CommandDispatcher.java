...
public class CommandDispatcher extends CommandHandler implements ICommandDispatcher {
	...
	public void a(ICommandListener icommandlistener, ICommand icommand, int i, String s, Object... aobject) {
		...
		if (flag) {
			...
			while (iterator.hasNext()) {
				...
				if (entityhuman != icommandlistener && minecraftserver.getPlayerList().isOp(entityhuman.getProfile())
						&& icommand.canUse(this.a, icommandlistener)) {
					...
					if (flag1 || flag2
							|| !(icommandlistener instanceof RemoteControlCommandListener) && !(icommandlistener instanceof MinecraftServer)) {
		...
		if (icommandlistener != minecraftserver && minecraftserver.worldServer[0].getGameRules().getBoolean("logAdminCommands")
				&& !cfg.getCommands().isSilentCommandblockConsole()) { // Spigot
