...
public class PlayerInteractManager {
	...
	public void a(BlockPosition blockposition, EnumDirection enumdirection) {
		// CraftBukkit start
		PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK, blockposition, enumdirection,
		...
		if (this.isCreative()) {
			...
			// this.world.douseFire((EntityHuman) null, blockposition,
			// enumdirection); // CraftBukkit - Moved down
			this.lastDigTick = this.currentTick;
			float f = 1.0F;

			// CraftBukkit start - Swings at air do *NOT* exist.
			if (event.useInteractedBlock() == Event.Result.DENY) {
				// If we denied a door from opening, we need to send a
				// correcting update to the client, as it already opened the
				// door.
				IBlockData data = this.world.getType(blockposition);
				if (block == blocks.WOODEN_DOOR) {
					// For some reason *BOTH* the bottom/top part have to be
					// marked updated.
					boolean bottom = data.get(BlockDoor.HALF) == BlockDoor.EnumDoorHalf.LOWER;
					((EntityPlayer) this.player).playerConnection.sendPacket(new PacketPlayOutBlockChange(this.world, blockposition));
					((EntityPlayer) this.player).playerConnection
							.sendPacket(new PacketPlayOutBlockChange(this.world, bottom ? blockposition.up() : blockposition.down()));
			...
			org.bukkit.event.block.BlockDamageEvent blockEvent = CraftEventFactory.callBlockDamageEvent(this.player, blockposition.getX(),
					blockposition.getY(), blockposition.getZ(), this.player.inventory.getItemInHand(), f >= 1.0f);
	...
	public boolean breakBlock(BlockPosition blockposition) {
		...
		if (this.player instanceof EntityPlayer) {
			...
			boolean isSwordNoBreak = this.gamemode.isCreative() && !this.player.getItemInMainHand().isEmpty()
					&& this.player.getItemInMainHand().getItem() instanceof ItemSword;

			// Tell client the block is gone immediately then process events
			// Don't tell the client if its a creative sword break because its
			// not broken!
			...
			if (nmsBlock != null && !event.isCancelled() && !this.isCreative() && this.player.hasBlock(nmsBlock.getBlockData())) {
				// Copied from block.a(World world, EntityHuman entityhuman,
				// BlockPosition blockposition, IBlockData iblockdata, @Nullable
			...
			if (event.isCancelled()) {
				...
				if (nmsBlock instanceof BlockDoor) {
					...
					((EntityPlayer) this.player).playerConnection
							.sendPacket(new PacketPlayOutBlockChange(world, bottom ? blockposition.up() : blockposition.down()));
				}
				// Update any tile entity data for this block
				TileEntity tileentity = this.world.getTileEntity(blockposition);
				if (tileentity != null) {
		...
		if (false && this.gamemode.isCreative() && !this.player.getItemInMainHand().isEmpty()
				&& this.player.getItemInMainHand().getItem() instanceof ItemSword) { // CraftBukkit
																						// -
																						// false
			return false;
		} else {
			...
			if (iblockdata.getBlock() == blocks.AIR)
				return false; // CraftBukkit - A plugin set block to air without
								// cancelling
			TileEntity tileentity = this.world.getTileEntity(blockposition);
			Block block = iblockdata.getBlock();

			// CraftBukkit start - Special case skulls, their item data comes
			// from a tile entity (Also check if block should drop items)
			...
			// And shulker boxes too for duplication on cancel reasons (Also
			...
			if ((block instanceof BlockCommand || block instanceof BlockStructure) && !this.player.isCreativeAndOp()) {
				...
				this.world.a(this.player, 2001, blockposition, blockRegistry.getCombinedId(iblockdata));
				// CraftBukkit start
	...
	public EnumInteractionResult a(EntityHuman entityhuman, World world, ItemStack itemstack, EnumHand enumhand, BlockPosition blockposition,
		...
		if (blockdata.getBlock() != blocks.AIR) {
			...
			PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(entityhuman, Action.RIGHT_CLICK_BLOCK, blockposition, enumdirection,
					itemstack, cancelledBlock, enumhand);
			firedInteract = true;
			...
			if (event.useInteractedBlock() == Event.Result.DENY) {
				...
				if (blockdata.getBlock() instanceof BlockDoor) {
					...
					((EntityPlayer) entityhuman).getBukkitEntity().sendHealthUpdate(); // SPIGOT-1341
																						// -
																						// reset
																						// health
																						// for
				...
				if (!entityhuman.isSneaking() || entityhuman.getItemInMainHand().isEmpty() && entityhuman.getItemInOffHand().isEmpty()) {
					...
					enuminteractionresult = iblockdata.getBlock().interact(world, blockposition, iblockdata, entityhuman, enumhand, enumdirection, f,
							f1, f2) ? EnumInteractionResult.SUCCESS : EnumInteractionResult.PASS;
				}
			}

			if (!itemstack.isEmpty() && enuminteractionresult != EnumInteractionResult.SUCCESS && !interactResult) { // add
																														// !interactResult
																														// SPIGOT-764
