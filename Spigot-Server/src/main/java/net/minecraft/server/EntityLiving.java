...
public abstract class EntityLiving extends Entity {
	...
	public EntityLiving(World world) {
		...
		this.initAttributes();
		// CraftBukkit - setHealth(getMaxHealth()) inlined and simplified to
		// skip the instanceof check for EntityPlayer, as getBukkitEntity() is
		// not initialized in constructor
		this.datawatcher.set(EntityLiving.HEALTH, (float) this.getAttributeInstance(attr.maxHealth).getValue());
		this.i = true;
	...
	protected void initAttributes() {
		this.getAttributeMap().b(attr.maxHealth);
		this.getAttributeMap().b(attr.c);
		this.getAttributeMap().b(attr.MOVEMENT_SPEED);
		this.getAttributeMap().b(attr.h);
		this.getAttributeMap().b(attr.i);
	}
	...
	public void Y() {
		...
		if (this.isAlive()) {
			if (this.a(Material.WATER)) {
				if (!this.bN() && !this.hasEffect(MobEffects.WATER_BREATHING) && !flag1) {
					...
					if (this.getAirTicks() == -20) {
						...
						for (int i = 0; i < 8; ++i) {
							...
							this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX + (double) f, this.locY + (double) f1,
				...
				// CraftBukkit start - Only set if needed to work around a
				// DataWatcher inefficiency
	...
	public int getExpReward() {
		...
		if (!this.world.isClientSide && (this.lastDamageByPlayerTime > 0 || this.alwaysGivesExp()) && this.isDropExperience()
	...
	protected void bO() {
		++this.deathTicks;
		if (this.deathTicks >= 20 && !this.dead) { // CraftBukkit -
													// (this.deathTicks == 20)
													// -> (this.deathTicks >= 20
													// && !this.dead)
			int i;

			// CraftBukkit start - Update getExpReward() above if the removed
			// if() changes!
			i = this.expToDrop;
			while (i > 0) {
			...
			for (i = 0; i < 20; ++i) {
				...
				this.world.addParticle(EnumParticle.EXPLOSION_NORMAL,
						...
						this.locZ + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, d0, d1, d2, new int[0]);
			}
		}
	...
	public void b(NBTTagCompound nbttagcompound) {
		...
		nbttagcompound.set("Attributes", attr.a(this.getAttributeMap()));
	...
	public void a(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKeyOfType("Attributes", 9) && this.world != null && !this.world.isClientSide) {
			attr.a(this.getAttributeMap(), nbttagcompound.getList("Attributes", 10));
		}
		...
		if (nbttagcompound.hasKey("Bukkit.MaxHealth")) {
			...
			if (nbtbase.getTypeId() == 5) {
				this.getAttributeInstance(attr.maxHealth).setValue(((NBTTagFloat) nbtbase).asDouble());
			} else if (nbtbase.getTypeId() == 3) {
				this.getAttributeInstance(attr.maxHealth).setValue(((NBTTagInt) nbtbase).asDouble());
			}
		}
		// CraftBukkit end
	...
	// CraftBukkit start - Delegate so we can handle providing a reason for
	// health being regained
		...
		if (this.isInvulnerable(damagesource)) {
			...
			if (this.getHealth() <= 0.0F) {
				...
				if (false && (damagesource == DamageSource.ANVIL || damagesource == DamageSource.FALLING_BLOCK)
						&& !this.getEquipment(EnumItemSlot.HEAD).isEmpty()) {
					this.getEquipment(EnumItemSlot.HEAD).damage((int) (f * 4.0F + this.random.nextFloat() * f * 2.0F), this);
					f *= 0.75F;
				}

				boolean flag = f > 0.0F && this.applyBlockingModifier(damagesource); // Copied
																						// from
																						// below
				...
				if ((float) this.noDamageTicks > (float) this.maxNoDamageTicks / 2.0F) {
					if (f <= this.lastDamage) {
						this.forceExplosionKnockback = true; // CraftBukkit -
																// SPIGOT-949 -
																// for vanilla
																// consistency,
																// cooldown does
	...
	public void a(Entity entity, float f, double d0, double d1) {
		if (this.random.nextDouble() >= this.getAttributeInstance(attr.c).getValue()) {
			this.impulse = true;
	...
	public void e(float f, float f1) {
		...
		if (i > 0) {
			...
			// this.damageEntity(DamageSource.FALL, (float) i); // CraftBukkit -
			// moved up
	...
	public int getArmorStrength() {
		AttributeInstance attributeinstance = this.getAttributeInstance(attr.h);
	...
	protected float applyArmorModifier(DamageSource damagesource, float f) {
		if (!damagesource.ignoresArmor()) {
			...
			f = CombatMath.a(f, (float) this.getArmorStrength(), (float) this.getAttributeInstance(attr.i).getValue());
		}
	...
	// CraftBukkit start
	protected boolean damageEntity0(final DamageSource damagesource, float f) { // void
																				// ->
																				// boolean,
																				// add
																				// final
		if (!this.isInvulnerable(damagesource)) {
			...
			EntityDamageEvent event = CraftEventFactory.handleLivingEntityDamageEvent(this, damagesource, originalDamage, hardHatModifier,
					blockingModifier, armorModifier, resistanceModifier, magicModifier, absorptionModifier, hardHat, blocking, armor, resistance,
					magic, absorption);
			if (event.isCancelled()) {
				return false;
			}
			...
			if ((damagesource == DamageSource.ANVIL || damagesource == DamageSource.FALLING_BLOCK) && this.getEquipment(EnumItemSlot.HEAD) != null) {
				this.getEquipment(EnumItemSlot.HEAD).damage((int) (event.getDamage() * 4.0F + this.random.nextFloat() * event.getDamage() * 2.0F),
						this);
			}
			...
			if (f > 0 || !human) {
				if (human) {
					// PAIL: Be sure to drag all this code from the EntityHuman
					// subclass each update.
	...
	public EntityLiving ci() {
		return (EntityLiving) (this.combatTracker.c() != null
				? this.combatTracker.c()
				: (this.killer != null ? this.killer : (this.lastDamager != null ? this.lastDamager : null)));
	}

	public final float getMaxHealth() {
		return (float) this.getAttributeInstance(attr.maxHealth).getValue();
	}
	...
	private int p() {
		return this.hasEffect(MobEffects.FASTER_DIG)
				? 6 - (1 + this.getEffect(MobEffects.FASTER_DIG).getAmplifier())
				: (this.hasEffect(MobEffects.SLOWER_DIG) ? 6 + (1 + this.getEffect(MobEffects.SLOWER_DIG).getAmplifier()) * 2 : 6);
	}
		...
		if (!this.au || this.aw >= this.p() / 2 || this.aw < 0) {
			...
			if (this.world instanceof WorldServer) {
				((WorldServer) this.world).getTracker().a((Entity) this,
						(Packet) (new PacketPlayOutAnimation(this, enumhand == EnumHand.MAIN_HAND ? 0 : 3)));
			}
		}
	...
	public void A(Entity entity) {
		...
		if (!(entity instanceof EntityBoat) && !(entity instanceof EntityHorseAbstract)) {
			...
			if (enumdirection != null) {
				...
				AxisAlignedBB axisalignedbb = new AxisAlignedBB(d3 - d5 / 2.0D, entity.getBoundingBox().b, d4 - d6 / 2.0D, d3 + d5 / 2.0D,
						Math.floor(entity.getBoundingBox().b) + (double) this.length, d4 + d6 / 2.0D);
				int[][] aint1 = aint;
				int i = aint.length;
				...
				for (int j = 0; j < i; ++j) {
					...
					} else if (!this.world.a(axisalignedbb1.d(0.0D, 1.0D, 0.0D))
							&& this.world.getType(new BlockPosition(d9, this.locY + 1.0D, d10)).q()) {
						d1 = d9;
						d2 = this.locY + 2.0D;
						d0 = d10;
					}
	...
	public void a(float f, float f1, float f2) {
		...
		if (this.cC() || this.bI()) {
			...
			if (this.isInWater() && (!(this instanceof EntityHuman) || !((EntityHuman) this).abilities.isFlying)) {
				...
				float f9 = 0.91F;
				BlockPosition.PooledBlockPosition blockposition_pooledblockposition = BlockPosition.PooledBlockPosition.d(this.locX,
						this.getBoundingBox().b - 1.0D, this.locZ);
				...
				f9 = 0.91F;
				if (this.onGround) {
					f9 = this.world.getType(blockposition_pooledblockposition.e(this.locX, this.getBoundingBox().b - 1.0D, this.locZ))
	...
	public boolean hasLineOfSight(Entity entity) {
		return this.world.rayTrace(new Vec3D(this.locX, this.locY + (double) this.getHeadHeight(), this.locZ),
				new Vec3D(entity.locX, entity.locY + (double) entity.getHeadHeight(), entity.locZ), false, true, false) == null;
	}
	...
	protected void ax() {
		this.velocityChanged = this.random.nextDouble() >= this.getAttributeInstance(attr.c).getValue();
	}
	...
	protected void v() {
		if (!this.activeItem.isEmpty() && this.isHandRaised()) {
			...
			if (this instanceof EntityPlayer) {
				...
				itemstack = (craftItem.equals(event.getItem()))
						? this.activeItem.a(this.world, this)
						: CraftItemStack.asNMSCopy(event.getItem()).a(world, this);
			} else {
