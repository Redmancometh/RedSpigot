...
public class ChunkRegionLoader implements IChunkLoader, IAsyncChunkSaver {
	...
	@Nullable
	protected Object[] a(World world, int i, int j, NBTTagCompound nbttagcompound) { // CraftBukkit
																						// -
																						// return
																						// Chunk
																						// ->
		...
		if (!nbttagcompound.hasKeyOfType("Level", 10)) {
			...
			if (!nbttagcompound1.hasKeyOfType("Sections", 9)) {
				...
				if (!chunk.a(i, j)) {
					ChunkRegionLoader.a.error("Chunk file at {},{} is in the wrong location; relocating. (Expected {}, {}, got {}, {})",
							Integer.valueOf(i), Integer.valueOf(j), Integer.valueOf(i), Integer.valueOf(j), Integer.valueOf(chunk.locX),
							Integer.valueOf(chunk.locZ));
					nbttagcompound1.setInt("xPos", i);
					nbttagcompound1.setInt("zPos", j);

					// CraftBukkit start - Have to move tile entities since we
					// don't load them at this stage
	...
	private synchronized boolean processSaveQueueEntry(boolean logCompletion) {
		...
		if (!iter.hasNext()) {
			...
			try {
				...
				this.b.remove(chunkcoordintpair, value); // CraftBukkit //
															// Spigot
			}
	...
	private static void saveBody(NBTTagCompound nbttagcompound, Chunk chunk, long worldTime, boolean worldHasSkyLight) { // Spigot
		...
		// Spigot start - End this method here and split off entity saving to
		// another method
	}
	...
	private void saveEntities(NBTTagCompound nbttagcompound, Chunk chunk, World world) {
		...
		if (list != null) {
			...
			while (iterator1.hasNext()) {
				...
				MinecraftKey minecraftkey = (MinecraftKey) blockRegistry.b(nextticklistentry.a());
	...
	private Chunk a(World world, NBTTagCompound nbttagcompound) {
		...
		// CraftBukkit start - End this method here and split off entity loading
		// to another method
		return chunk;
	}
		...
		if (nbttagcompound.hasKeyOfType("TileTicks", 9)) {
			...
			for (int j1 = 0; j1 < nbttaglist3.size(); ++j1) {
				...
				if (nbttagcompound4.hasKeyOfType("i", 8)) {
					block = blockRegistry.getByName(nbttagcompound4.getString("i"));
				} else {
					block = blockRegistry.getById(nbttagcompound4.getInt("i"));
				}

				world.b(new BlockPosition(nbttagcompound4.getInt("x"), nbttagcompound4.getInt("y"), nbttagcompound4.getInt("z")), block,
						nbttagcompound4.getInt("t"), nbttagcompound4.getInt("p"));
			}
		}
	...
	public static Entity spawnEntity(NBTTagCompound nbttagcompound, World world, double d0, double d1, double d2, boolean flag,
