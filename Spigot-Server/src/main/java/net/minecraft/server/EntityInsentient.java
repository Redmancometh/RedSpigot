...
public abstract class EntityInsentient extends EntityLiving {
	...
	protected void initAttributes() {
		...
		this.getAttributeMap().b(attr.FOLLOW_RANGE).setValue(16.0D);
	}
	...
	public boolean setGoalTarget(EntityLiving entityliving, EntityTargetEvent.TargetReason reason, boolean fireEvent) {
		...
		if (fireEvent) {
			...
			if (reason == EntityTargetEvent.TargetReason.UNKNOWN) {
				world.getServer().getLogger().log(java.util.logging.Level.WARNING, "Unknown target reason, please report on the issue tracker",
						new Exception());
			}
		...
		if (this.world.isClientSide) {
			for (int i = 0; i < 20; ++i) {
				...
				this.world.addParticle(EnumParticle.EXPLOSION_NORMAL,
						this.locX + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width - d0 * 10.0D,
						this.locY + (double) (this.random.nextFloat() * this.length) - d1 * 10.0D,
						this.locZ + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width - d2 * 10.0D, d0, d1, d2,
						new int[0]);
			}
		} else {
	...
	public static void a(DataConverterManager dataconvertermanager, Class<?> oclass) {
		dataconvertermanager.a(DataConverterTypes.ENTITY,
				(DataInspector) (new DataInspectorItemList(oclass, new String[]{"ArmorItems", "HandItems"})));
	}
	...
	public void a(NBTTagCompound nbttagcompound) {
		...
		// CraftBukkit start - If looting or persistence is false only use it if
		// it was set after we started using it
	...
	protected void a(EntityItem entityitem) {
		...
		EntityPickupItemEvent entityEvent = new EntityPickupItemEvent((LivingEntity) getBukkitEntity(),
	...
	protected void L() {
		if (this.persistent) {
			...
			if (entityhuman != null) {
				...
				if (d3 > 16384.0D) { // CraftBukkit - remove
										// isTypeNotPersistent() check
					this.die();
				}

				if (this.ticksFarFromPlayer > 600 && this.random.nextInt(800) == 0 && d3 > 1024.0D) { // CraftBukkit
																										// -
																										// remove
																										// isTypeNotPersistent()
																										// check
	...
	public boolean canSpawn() {
		return !this.world.containsLiquid(this.getBoundingBox()) && this.world.getCubes(this, this.getBoundingBox()).isEmpty()
				&& this.world.a(this.getBoundingBox(), (Entity) this);
	}
	...
	protected void dropEquipment(boolean flag, int i) {
		...
		for (int k = 0; k < j; ++k) {
			...
			if (!itemstack.isEmpty() && !EnchantmentManager.shouldNotDrop(itemstack) && (flag || flag1)
					&& (double) (this.random.nextFloat() - (float) i * 0.01F) < d0) {
	...
	public EnumItemSlot d(ItemStack itemstack) {
		return itemstack.getItem() != items.getItemOf(blocks.PUMPKIN) && itemstack.getItem() != Items.SKULL
				? (itemstack.getItem() instanceof ItemArmor
						? ((ItemArmor) itemstack.getItem()).c
						: (itemstack.getItem() == Items.cS
								? EnumItemSlot.CHEST
								: (itemstack.getItem() == Items.SHIELD ? EnumItemSlot.OFFHAND : EnumItemSlot.MAINHAND)))
				: EnumItemSlot.HEAD;
	}
	...
	public GroupDataEntity prepare(DifficultyDamageScaler difficultydamagescaler, @Nullable GroupDataEntity groupdataentity) {
		this.getAttributeInstance(attr.FOLLOW_RANGE).b(new AttributeModifier("Random spawn bonus", this.random.nextGaussian() * 0.05D, 1));
	...
	public boolean b(EnumItemSlot enumitemslot, ItemStack itemstack) {
		...
		return enumitemslot1 == enumitemslot || enumitemslot1 == EnumItemSlot.MAINHAND && enumitemslot == EnumItemSlot.OFFHAND
				|| enumitemslot1 == EnumItemSlot.OFFHAND && enumitemslot == EnumItemSlot.MAINHAND;
	}
