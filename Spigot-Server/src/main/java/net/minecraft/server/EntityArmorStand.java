...
public class EntityArmorStand extends EntityLiving {
	...
	private static final Predicate<Entity> bx = new Predicate() {
		public boolean a(@Nullable Entity entity) {
			return entity instanceof EntityMinecartAbstract
					&& ((EntityMinecartAbstract) entity).v() == EntityMinecartAbstract.EnumMinecartType.RIDEABLE;
		}
	...
	public static void a(DataConverterManager dataconvertermanager) {
		dataconvertermanager.a(DataConverterTypes.ENTITY,
				(DataInspector) (new DataInspectorItemList(EntityArmorStand.class, new String[]{"ArmorItems", "HandItems"})));
	}
	...
	private void a(EntityHuman entityhuman, EnumItemSlot enumitemslot, ItemStack itemstack, EnumHand enumhand) {
		...
		if (itemstack1.isEmpty() || (this.bB & 1 << enumitemslot.c() + 8) == 0) {
			if (!itemstack1.isEmpty() || (this.bB & 1 << enumitemslot.c() + 16) == 0) {
				...
				PlayerArmorStandManipulateEvent armorStandManipulateEvent = new PlayerArmorStandManipulateEvent(player, self, playerHeldItem,
						armorStandItem, slot);
	...
	public boolean damageEntity(DamageSource damagesource, float f) {
		...
		if (!this.world.isClientSide && !this.dead) {
			if (DamageSource.OUT_OF_WORLD.equals(damagesource)) {
				this.killEntity(); // CraftBukkit - this.die() ->
									// this.killEntity()
				return false;
	...
	private void D() {
		if (this.world instanceof WorldServer) {
			((WorldServer) this.world).a(EnumParticle.BLOCK_DUST, this.locX, this.locY + (double) this.length / 1.5D, this.locZ, 10,
					(double) (this.width / 4.0F), (double) (this.length / 4.0F), (double) (this.width / 4.0F), 0.05D,
					new int[]{blockRegistry.getCombinedId(blocks.PLANKS.getBlockData())});
		}
	...
	private void E() {
		drops.add(org.bukkit.craftbukkit.inventory.CraftItemStack.asBukkitCopy(new ItemStack(Items.ARMOR_STAND))); // CraftBukkit
																													// -
																													// add
																													// to
																													// drops
	...
	private void F() {
		...
		for (i = 0; i < this.by.size(); ++i) {
			...
			if (!itemstack.isEmpty()) {
				drops.add(org.bukkit.craftbukkit.inventory.CraftItemStack.asBukkitCopy(itemstack)); // CraftBukkit
																									// -
																									// add
																									// to
																									// drops
	...
	public void killEntity() {
		org.bukkit.craftbukkit.event.CraftEventFactory.callEntityDeathEvent(this, drops); // CraftBukkit
																							// -
																							// call
																							// event
		this.die();
