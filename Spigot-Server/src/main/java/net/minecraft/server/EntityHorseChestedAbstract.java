...
public abstract class EntityHorseChestedAbstract extends EntityHorseAbstract {
	...
	public void die(DamageSource damagesource) {
		...
		if (this.isCarryingChest()) {
			if (!this.world.isClientSide) {
				this.a(items.getItemOf(blocks.CHEST), 1);
			}
	...
	public boolean c(int i, ItemStack itemstack) {
		if (i == 499) {
			...
			if (!this.isCarryingChest() && itemstack.getItem() == items.getItemOf(blocks.CHEST)) {
	...
	public boolean a(EntityHuman entityhuman, EnumHand enumhand) {
		...
		if (itemstack.getItem() == Items.SPAWN_EGG) {
			...
			if (!itemstack.isEmpty()) {
				...
				if (!flag && !this.isCarryingChest() && itemstack.getItem() == items.getItemOf(blocks.CHEST)) {
					this.setCarryingChest(true);
					this.dp();
					flag = true;
					this.loadChest();
