...
public abstract class BlockDiodeAbstract extends BlockFacingHorizontal {
	...
	public void b(World world, BlockPosition blockposition, IBlockData iblockdata, Random random) {
		if (!this.b((IBlockAccess) world, blockposition, iblockdata)) {
			...
			if (this.d && !flag) {
				// CraftBukkit start
				if (CraftEventFactory.callRedstoneChange(world, blockposition.getX(), blockposition.getY(), blockposition.getZ(), 15, 0)
						.getNewCurrent() != 0) {
					return;
				}
				// CraftBukkit end
				...
				// CraftBukkit start
				if (CraftEventFactory.callRedstoneChange(world, blockposition.getX(), blockposition.getY(), blockposition.getZ(), 0, 15)
						.getNewCurrent() != 15) {
					return;
				}
				// CraftBukkit end
	...
	public int b(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition, EnumDirection enumdirection) {
		return !this.A(iblockdata)
				? 0
				: (iblockdata.get(BlockDiodeAbstract.FACING) == enumdirection ? this.a(iblockaccess, blockposition, iblockdata) : 0);
	}
	...
	protected int c(IBlockAccess iblockaccess, BlockPosition blockposition, IBlockData iblockdata) {
		...
		return Math.max(this.a(iblockaccess, blockposition.shift(enumdirection1), enumdirection1),
				this.a(iblockaccess, blockposition.shift(enumdirection2), enumdirection2));
	}
	...
	protected int a(IBlockAccess iblockaccess, BlockPosition blockposition, EnumDirection enumdirection) {
		...
		return this.B(iblockdata)
				? (block == blocks.REDSTONE_BLOCK
						? 15
						: (block == blocks.REDSTONE_WIRE
								? ((Integer) iblockdata.get(BlockRedstoneWire.POWER)).intValue()
	...
	public boolean isDiode(IBlockData iblockdata) {
