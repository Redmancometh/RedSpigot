...
public class BlockCrops extends BlockPlant implements IBlockFragilePlantElement {
	...
	private static final AxisAlignedBB[] a = new AxisAlignedBB[]{new AxisAlignedBB(0.0D, 0.0D, 0.0D, 1.0D, 0.125D, 1.0D),
		...
		if (world.getLightLevel(blockposition.up()) >= 9) {
			...
			if (i < this.g()) {
				...
				if (random.nextInt((int) ((100.0F / world.spigotConfig.wheatModifier) * (25.0F / f)) + 1) == 0) { // Spigot
					...
					CraftEventFactory.handleBlockGrowEvent(world, blockposition.getX(), blockposition.getY(), blockposition.getZ(), this,
							toLegacyData(data));
					// CraftBukkit end
				}
			}
	...
	protected static float a(Block block, World world, BlockPosition blockposition) {
		...
		if (flag && flag1) {
			...
			boolean flag2 = block == world.getType(blockposition4.north()).getBlock() || block == world.getType(blockposition5.north()).getBlock()
					|| block == world.getType(blockposition5.south()).getBlock() || block == world.getType(blockposition4.south()).getBlock();
