...
public class Chunk {
		...
		switch (radius) {
			...
			case 1 :
				final int mask =
						...
						(0x1 << (1 * 5 + 1 + 12)) | (0x1 << (0 * 5 + 1 + 12)) | (0x1 << (-1 * 5 + 1 + 12)) | (0x1 << (1 * 5 + 0 + 12))
								| (0x1 << (0 * 5 + 0 + 12)) | (0x1 << (-1 * 5 + 0 + 12)) | (0x1 << (1 * 5 + -1 + 12)) | (0x1 << (0 * 5 + -1 + 12))
								| (0x1 << (-1 * 5 + -1 + 12));
				...
				for (int j = 0; j < 16; ++j) {
					if (this.i[i + j * 16]) {
						...
						for (iterator = EnumDirection.EnumDirectionLimit.HORIZONTAL.iterator(); iterator
								.hasNext(); j1 = Math.min(j1, this.world.d(l + enumdirection.getAdjacentX(), i1 + enumdirection.getAdjacentZ()))) {
	...
	public IBlockData a(BlockPosition blockposition, IBlockData iblockdata) {
		...
		if (iblockdata1 == iblockdata) {
			...
			if (chunksection.getType(i, j & 15, k).getBlock() != block) {
				...
				// BlockPlaceEvent, unless it's a BlockContainer. Prevents
				// blocks such as TNT from activating when cancelled.
	...
	public int getBrightness(EnumSkyBlock enumskyblock, BlockPosition blockposition) {
		...
		return chunksection == Chunk.a
				? (this.c(blockposition) ? enumskyblock.c : 0)
				: (enumskyblock == EnumSkyBlock.SKY
						? (!this.world.worldProvider.m() ? 0 : chunksection.b(i, j & 15, k))
						: (enumskyblock == EnumSkyBlock.BLOCK ? chunksection.c(i, j & 15, k) : enumskyblock.c));
	}
	...
	public void a(Entity entity) {
		...
		if (i != this.locX || j != this.locZ) {
			Chunk.e.warn("Wrong location! ({}, {}) should be ({}, {}), {}", Integer.valueOf(i), Integer.valueOf(j), Integer.valueOf(this.locX),
					Integer.valueOf(this.locZ), entity);
			entity.die();
		}
	...
	public void a(BlockPosition blockposition, TileEntity tileentity) {
		...
		if (this.getBlockData(blockposition).getBlock() instanceof ITileEntity) {
			...
			System.out.println("Attempted to place a tile entity (" + tileentity + ") at " + tileentity.position.getX() + ","
					...
					+ org.bukkit.craftbukkit.util.CraftMagicNumbers.getMaterial(getBlockData(blockposition).getBlock())
	...
	public void removeEntities() {
		...
		for (int j = 0; j < i; ++j) {
			...
			while (iter.hasNext()) {
				...
				// Do not pass along players, as doing so can get them stuck
				// outside of time.
				// (which for example disables inventory icon updates and
					...
					if (entity1.getBoundingBox().c(axisalignedbb) && entity1 != entity) {
						...
						if (aentity != null) {
							...
							for (int i1 = 0; i1 < l; ++i1) {
								...
								if (entity2 != entity && entity2.getBoundingBox().c(axisalignedbb)
										&& (predicate == null || predicate.apply(entity2))) {
									list.add(entity2);
								}
							}
						}
	...
	public <T extends Entity> void a(Class<? extends T> oclass, AxisAlignedBB axisalignedbb, List<T> list, Predicate<? super T> predicate) {
		...
		for (int k = i; k <= j; ++k) {
			...
			while (iterator.hasNext()) {
				...
				if (oclass.isInstance(entity) && entity.getBoundingBox().c(axisalignedbb) && (predicate == null || predicate.apply((T) entity))) { // CraftBukkit
																																					// -
																																					// fix
																																					// decompile
																																					// error
	...
	public boolean a(boolean flag) {
		...
		} else if (this.t && this.world.getTime() >= this.lastSaved + MinecraftServer.getServer().autosavePeriod * 4) { // Spigot
																														// -
																														// Only
																														// save
																														// if
	...
	public void b(boolean flag) {
		...
		this.r = true;
		if (!this.lit && this.done && this.world.spigotConfig.randomLightUpdates) { // Spigot
																					// -
																					// also
																					// use
																					// random
	...
	public boolean isReady() {
		// Spigot Start
		/*
		 * As of 1.7, Mojang added a check to make sure that only chunks which
		 * have been lit are sent to the client. Unfortunately this interferes
		 * with our modified chunk ticking algorithm, which will only tick
		 * chunks distant from the player on a very infrequent basis. We cannot
		 * unfortunately do this lighting stage during chunk gen as it appears
		 * to put a lot more noticeable load on the server, than when it is done
		 * at play time. For now at least we will simply send all chunks, in
	...
	public void a(ChunkSection[] achunksection) {
		if (this.sections.length != achunksection.length) {
			Chunk.e.warn("Could not set level chunk sections, array length is {} instead of {}", Integer.valueOf(achunksection.length),
					Integer.valueOf(this.sections.length));
		} else {
	...
	public void a(byte[] abyte) {
		if (this.g.length != abyte.length) {
			Chunk.e.warn("Could not set level chunk biomes, array length is {} instead of {}", Integer.valueOf(abyte.length),
		...
		if (this.x < 4096) {
			...
			for (int i = 0; i < 8; ++i) {
				...
				for (int i1 = 0; i1 < 16; ++i1) {
					...
					if (this.sections[j] == Chunk.a && flag
							|| this.sections[j] != Chunk.a && this.sections[j].getType(k, i1, l).getMaterial() == Material.AIR) {
	...
	private boolean e(int i, int j) {
		...
		boolean flag = false;
		boolean flag1 = false;
		BlockPosition.MutableBlockPosition blockposition_mutableblockposition = new BlockPosition.MutableBlockPosition((this.locX << 4) + i, 0,
				(this.locZ << 4) + j);
	...
	public void a(int[] aint) {
		if (this.heightMap.length != aint.length) {
			Chunk.e.warn("Could not set level chunk heightmap, array length is {} instead of {}", Integer.valueOf(aint.length),
