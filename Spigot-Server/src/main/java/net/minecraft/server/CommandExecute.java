...
public class CommandExecute extends CommandAbstract {
	...
	public void execute(MinecraftServer minecraftserver, ICommandListener icommandlistener, String[] astring) throws CommandException {
		if (astring.length < 5) {
			...
			CommandListenerWrapper commandlistenerwrapper = CommandListenerWrapper.a(icommandlistener).a(entity, new Vec3D(d0, d1, d2))
					.a(minecraftserver.worlds.get(0).getGameRules().getBoolean("commandBlockOutput")); // CraftBukkit
			ICommandHandler icommandhandler = minecraftserver.getCommandHandler();

			try {
				// CraftBukkit start
				org.bukkit.command.CommandSender sender = CommandBlockListenerAbstract.unwrapSender(icommandlistener);
				int i = CommandBlockListenerAbstract.executeCommand(commandlistenerwrapper,
						new ProxiedNativeCommandSender(commandlistenerwrapper, sender, entity.getBukkitEntity()), s);
				// CraftBukkit end
	...
	public List<String> tabComplete(MinecraftServer minecraftserver, ICommandListener icommandlistener, String[] astring,
			@Nullable BlockPosition blockposition) {
		return astring.length == 1
				? a(astring, minecraftserver.getPlayers())
				: (astring.length > 1 && astring.length <= 4
						...
						: (astring.length > 5 && astring.length <= 8 && "detect".equals(astring[4])
								...
								: (astring.length == 9 && "detect".equals(astring[4])
										? a(astring, (Collection) blockRegistry.keySet())
										: Collections.<String>emptyList()))); // CraftBukkit
																				// -
																				// decompile
																				// error
	}
