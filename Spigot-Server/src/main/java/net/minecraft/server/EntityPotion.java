...
public class EntityPotion extends EntityProjectile {
	...
	protected void a(MovingObjectPosition movingobjectposition) {
		if (!this.world.isClientSide) {
			...
			if (flag) {
				this.n();
			} else if (true || !list.isEmpty()) { // CraftBukkit - Call event
													// even if no effects to
													// apply
				if (this.isLingering()) {
	...
	private void a(MovingObjectPosition movingobjectposition, List<MobEffect> list) {
		...
		if (!event.isCancelled() && list != null && !list.isEmpty()) { // do not
																		// process
																		// effects
																		// if
																		// there
			...
			for (LivingEntity victim : event.getAffectedEntities()) {
				...
				while (iterator1.hasNext()) {
					...
					// CraftBukkit start - Abide by PVP settings - for players
					// only!
					if (!this.world.pvpMode && this.getShooter() instanceof EntityPlayer && entityliving instanceof EntityPlayer
							&& entityliving != this.getShooter()) {
						int i = effectRegistry.a(mobeffectlist);
						// Block SLOWER_MOVEMENT, SLOWER_DIG, HARM, BLINDNESS,
						// HUNGER, WEAKNESS and POISON potions
					...
					if (mobeffectlist.isInstant()) {
						...
						if (i > 20) {
							entityliving.addEffect(
									new MobEffect(mobeffectlist, i, mobeffect.getAmplifier(), mobeffect.isAmbient(), mobeffect.isShowParticles()));
						}
					}
				}
			}
	...
	private void a(ItemStack itemstack, PotionRegistry potionregistry) {
		...
		// CraftBukkit start
		org.bukkit.event.entity.LingeringPotionSplashEvent event = org.bukkit.craftbukkit.event.CraftEventFactory.callLingeringPotionSplashEvent(this,
