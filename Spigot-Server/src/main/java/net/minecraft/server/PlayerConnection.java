...
public class PlayerConnection implements PacketListenerPlayIn, ITickable {
	...
	private final static HashSet<Integer> invalidItems = new HashSet<Integer>(java.util.Arrays.asList(8, 9, 10, 11, 26, 34, 36, 43, 51, 55, 59, 62,
			63, 64, 68, 71, 74, 75, 83, 90, 92, 93, 94, 104, 105, 115, 117, 118, 119, 125, 127, 132, 140, 141, 142, 144)); // TODO:
																															// Check
																															// after
																															// every
																															// update.
	...
	public void e() {
		...
		if (this.player.J() > 0L && this.minecraftServer.getIdleTimeout() > 0
				&& MinecraftServer.aw() - this.player.J() > (long) (this.minecraftServer.getIdleTimeout() * 1000 * 60)) {
	...
	public void disconnect(String s) {
		...
		this.networkManager.sendPacket(new PacketPlayOutKickDisconnect(chatcomponenttext), new GenericFutureListener() {
			public void operationComplete(Future future) throws Exception { // CraftBukkit
																			// -
																			// decompile
																			// error
	...
	private static boolean b(PacketPlayInVehicleMove packetplayinvehiclemove) {
		return !Doubles.isFinite(packetplayinvehiclemove.getX()) || !Doubles.isFinite(packetplayinvehiclemove.getY())
				|| !Doubles.isFinite(packetplayinvehiclemove.getZ()) || !Floats.isFinite(packetplayinvehiclemove.getPitch())
			...
			if (entity != this.player && entity.bE() == this.player && entity == this.r) {
				...
				if (i > Math.max(this.allowedPlayerTicks, 5)) {
					PlayerConnection.LOGGER
							.debug(this.player.getName() + " is sending move packets too frequently (" + i + " packets since last tick)");
					i = 1;
				}
				...
				speed *= 2f; // TODO: Get the speed of the vehicle instead of
								// the player

				if (d10 - d9 > Math.max(100.0D, Math.pow((double) (cfg.getMovedTooQuicklyMultiplier() * (float) i * speed), 2))
						&& (!this.minecraftServer.R() || !this.minecraftServer.Q().equals(entity.getName()))) { // Spigot
					// CraftBukkit end
					PlayerConnection.LOGGER.warn("{} (vehicle of {}) moved too quickly! {},{},{}", entity.getName(), this.player.getName(),
				...
				if (d10 > cfg.getMovedWronglyThreshhold()) { // Spigot
					flag1 = true;
				...
				// If the packet contains movement information then we update
				// the To location with the correct XYZ.
				...
				if ((delta > 1f / 256 || deltaAngle > 10f) && !this.player.isFrozen()) {
					...
					if (true) { // Spigot - don't skip any move events
						...
						// teleport the Player
						// there to avoid any 'Moved wrongly' or 'Moved too
						// quickly' errors.
						...
						// Check to see if the Players Location has some how
						// changed during the call of the event.
						// This can happen due to a plugin teleporting the
						// player instead of using .setTo()
			...
			if (!this.player.viewingCredits && !this.player.isFrozen()) { // CraftBukkit
				...
				if (this.teleportPos != null) {
					...
					if (this.player.isPassenger()) {
						this.player.setLocation(this.player.locX, this.player.locY, this.player.locZ, packetplayinflying.a(this.player.yaw),
								packetplayinflying.b(this.player.pitch));
						this.minecraftServer.getPlayerList().d(this.player);
						this.allowedPlayerTicks = 20; // CraftBukkit
					} else {
						// CraftBukkit - Make sure the move is valid but then
						// reset it for plugins to modify
						...
						if (this.player.isSleeping()) {
							if (d11 > 1.0D) {
								this.a(this.player.locX, this.player.locY, this.player.locZ, packetplayinflying.a(this.player.yaw),
										packetplayinflying.b(this.player.pitch));
							}
							...
							// CraftBukkit start - handle custom speeds and
							// skipped ticks
							this.allowedPlayerTicks += (System.currentTimeMillis() / 50) - this.lastTick;
							this.allowedPlayerTicks = Math.max(this.allowedPlayerTicks, 1);
							this.lastTick = (int) (System.currentTimeMillis() / 50);
							...
							if (i > Math.max(this.allowedPlayerTicks, 5)) {
								PlayerConnection.LOGGER.debug("{} is sending move packets too frequently ({} packets since last tick)",
							...
							if (!this.player.L() && (!this.player.x().getGameRules().getBoolean("disableElytraMovementCheck") || !this.player.cP())) {
								...
								if (d11 - d10 > Math.max(f2, Math.pow((double) (cfg.getMovedTooQuicklyMultiplier() * (float) i * speed), 2))
										&& (!this.minecraftServer.R() || !this.minecraftServer.Q().equals(this.player.getName()))) { // Spigot
									// CraftBukkit end
									PlayerConnection.LOGGER.warn("{} moved too quickly! {},{},{}", this.player.getName(), Double.valueOf(d7),
											Double.valueOf(d8), Double.valueOf(d9));
							...
							if (!this.player.L() && d11 > cfg.getMovedWronglyThreshhold() && !this.player.isSleeping()
							...
							Location from = new Location(player.getWorld(), lastPosX, lastPosY, lastPosZ, lastYaw, lastPitch); // Get
																																...
																																// Event
																																// location.
							Location to = player.getLocation().clone(); // Start
																		// off
																		// the
																		// To
																		// location
							...
							// If the packet contains movement information then
							// we update the To location with the correct XYZ.
							...
							// If the packet contains look information then we
							// update the To location with the correct Yaw &
							// Pitch.
							...
							// Prevent 40 event-calls for less than a single
							// pixel of movement >.>
							double delta = Math.pow(this.lastPosX - to.getX(), 2) + Math.pow(this.lastPosY - to.getY(), 2)
									+ Math.pow(this.lastPosZ - to.getZ(), 2);
							float deltaAngle = Math.abs(this.lastYaw - to.getYaw()) + Math.abs(this.lastPitch - to.getPitch());
							...
							if ((delta > 1f / 256 || deltaAngle > 10f) && !this.player.isFrozen()) {
								...
								if (from.getX() != Double.MAX_VALUE) {
									...
									// If the event is cancelled we move the
									// player back to their old location.
									if (event.isCancelled()) {
										teleport(from);
										return;
									}

									// If a Plugin has changed the To
									// destination then we teleport the Player
									// there to avoid any 'Moved wrongly' or
									// 'Moved too quickly' errors.
									// We only do this if the Event was not
									// cancelled.
									if (!oldTo.equals(event.getTo()) && !event.isCancelled()) {
										this.player.getBukkitEntity().teleport(event.getTo(), PlayerTeleportEvent.TeleportCause.PLUGIN);
										return;
									...
									// Check to see if the Players Location has
									// some how changed during the call of the
									// event.
									// This can happen due to a plugin
									// teleporting the player instead of using
									// .setTo()
									if (!from.equals(this.getPlayer().getLocation()) && this.justTeleported) {
										this.justTeleported = false;
										return;
							...
							this.player.setLocation(d4, d5, d6, f, f1); // Copied
																		// from
																		// above
							// CraftBukkit end
		...
		switch (packetplayinblockdig.c()) {
			case SWAP_HELD_ITEMS :
				if (!this.player.isSpectator()) {
					...
					// CraftBukkit start
					PlayerSwapHandItemsEvent swapItemsEvent = new PlayerSwapHandItemsEvent(getPlayer(), CraftItemStack.asBukkitCopy(itemstack),
			...
			case DROP_ITEM :
				if (!this.player.isSpectator()) {
					...
					// If the ticks aren't the same then the count starts from 0
					// and we update the lastDropTick.
					if (this.lastDropTick != MinecraftServer.currentTick) {
						this.dropCount = 0;
						this.lastDropTick = MinecraftServer.currentTick;
					} else {
						// Else we increment the drop count and check the
						// amount.
						this.dropCount++;
				...
				if (d3 > 36.0D) {
					...
					if (packetplayinblockdig.c() == PacketPlayInBlockDig.EnumPlayerDigType.START_DESTROY_BLOCK) {
						if (!this.minecraftServer.a(worldserver, blockposition, this.player) && worldserver.getWorldBorder().a(blockposition)) {
							...
							CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK, blockposition, packetplayinblockdig.b(),
	...
	public void a(PacketPlayInUseItem packetplayinuseitem) {
		...
		if (blockposition.getY() >= this.minecraftServer.getMaxBuildHeight() - 1
				&& (enumdirection == EnumDirection.UP || blockposition.getY() >= this.minecraftServer.getMaxBuildHeight())) {
			...
			this.player.playerConnection.sendPacket(new PacketPlayOutChat(chatmessage, ChatMessageType.GAME_INFO));
				...
				&& !this.minecraftServer.a(worldserver, blockposition, this.player) && worldserver.getWorldBorder().a(blockposition)) {
			...
			double reachDistance = NumberConversions.square(eyeLoc.getX() - blockposition.getX())
					+ NumberConversions.square(eyeLoc.getY() - blockposition.getY()) + NumberConversions.square(eyeLoc.getZ() - blockposition.getZ());
			...
			// CraftBukkit end
			this.player.playerInteractManager.a(this.player, worldserver, itemstack, enumhand, blockposition, enumdirection, packetplayinuseitem.d(),
					packetplayinuseitem.e(), packetplayinuseitem.f());
		}
	...
	public void a(PacketPlayInBlockPlace packetplayinblockplace) {
		...
		if (!itemstack.isEmpty()) {
			...
			if (movingobjectposition == null || movingobjectposition.type != MovingObjectPosition.EnumMovingObjectType.BLOCK) {
				org.bukkit.event.player.PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(this.player, Action.RIGHT_CLICK_AIR,
						itemstack, enumhand);
				...
				if (player.playerInteractManager.firedInteract) {
					...
					org.bukkit.event.player.PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(player, Action.RIGHT_CLICK_BLOCK,
							movingobjectposition.a(), movingobjectposition.direction, itemstack, true, enumhand);
					cancelled = event.useItemInHand() == Event.Result.DENY;
				}
			}
	...
	public void a(PacketPlayInSpectate packetplayinspectate) {
		...
		if (this.player.isSpectator()) {
			...
			if (entity != null) {
				...
				/*
				 * CraftBukkit start - replace with bukkit handling for
				 * multi-world if (entity.world == this.player.world) {
				 * this.player.enderTeleportTo(entity.locX, entity.locY,
				 * entity.locZ); } else { WorldServer worldserver1 =
				 * this.player.x(); WorldServer worldserver2 = (WorldServer)
				 * entity.world;
				 * 
				 * this.player.dimension = entity.dimension; this.sendPacket(new
				 * PacketPlayOutRespawn(this.player.dimension,
				 ...
				 * worldserver1.removeEntity(this.player); this.player.dead =
				 * false; this.player.setPositionRotation(entity.locX,
				 * entity.locY, entity.locZ, entity.yaw, entity.pitch); if
				 * (this.player.isAlive()) {
				 ...
				 * this.minecraftServer.getPlayerList().a(this.player,
				 * worldserver1); this.player.enderTeleportTo(entity.locX,
				 * entity.locY, entity.locZ);
				 * this.player.playerInteractManager.a(worldserver2);
				 * this.minecraftServer.getPlayerList().b(this.player,
				 * worldserver2);
	...
	public void a(IChatBaseComponent ichatbasecomponent) {
		...
		// CraftBukkit start - Replace vanilla quit message handling with our
		// own.
		/*
	...
	public void a(PacketPlayInChat packetplayinchat) {
		...
		if (this.player.dead || this.player.getChatFlags() == EntityHuman.EnumChatVisibility.HIDDEN) { // CraftBukkit
			...
			if (isSync) {
				...
				// Spigot end
			} else if (this.player.getChatFlags() == EntityHuman.EnumChatVisibility.SYSTEM) { // Re-add
																								// "Command
																								// Only"
																								// flag
																								// check
			...
			for (String exclude : cfg.getCommands().getSpamExclusions()) {
		...
		if (!async && s.startsWith("/")) {
			...
			if (PlayerChatEvent.getHandlerList().getRegisteredListeners().length != 0) {
				...
				try {
					...
					Thread.currentThread().interrupt(); // This is proper habit
														// for java. If we
														// aren't handling it,
														// pass it on!
				} catch (ExecutionException e) {
	...
	private void handleCommand(String s) {
		...
		if (cfg.getCommands().isLog()) // Spigot
				...
				if (packetplayinuseentity.a() == PacketPlayInUseEntity.EnumEntityUseAction.INTERACT
					...
					if (packetplayinuseentity.a() == PacketPlayInUseEntity.EnumEntityUseAction.INTERACT) {
						event = new PlayerInteractEntityEvent((Player) this.getPlayer(), entity.getBukkitEntity(),
								(packetplayinuseentity.b() == EnumHand.OFF_HAND) ? EquipmentSlot.OFF_HAND : EquipmentSlot.HAND);
					} else {
		...
		switch (packetplayinclientcommand_enumclientcommand) {
			case PERFORM_RESPAWN :
				if (this.player.viewingCredits) {
					...
					// this.player =
					// this.minecraftServer.getPlayerList().moveToWorld(this.player,
					// 0, true);
					this.minecraftServer.getPlayerList().changeDimension(this.player, 0, PlayerTeleportEvent.TeleportCause.END_PORTAL); // CraftBukkit
																																		// -
																																		// reroute
																																		// logic
																																		// through
	...
	public void a(PacketPlayInWindowClick packetplayinwindowclick) {
		...
		if (this.player.activeContainer.windowId == packetplayinwindowclick.a() && this.player.activeContainer.c(this.player)
				&& this.player.activeContainer.canUse(this.player)) { // CraftBukkit
			boolean cancelled = this.player.isSpectator(); // CraftBukkit - see
															// below if
				...
				switch (packetplayinwindowclick.f()) {
					case PICKUP :
						...
						if (packetplayinwindowclick.c() == 0 || packetplayinwindowclick.c() == 1) {
							action = InventoryAction.NOTHING; // Don't want to
																// repeat
																// ourselves
										...
										if (cursor.isEmpty()) {
											...
											if (clickedItem.doMaterialsMatch(cursor) && ItemStack.equals(clickedItem, cursor)) {
												...
												if (toPlace == 1) {
													...
													action = toPlace != -1 ? InventoryAction.PICKUP_SOME : InventoryAction.PICKUP_ONE; // this
																																		// happens
																																		// with
																																		// oversized
																																		// stacks
					...
					case THROW :
						if (packetplayinwindowclick.b() >= 0) {
							...
							// Sane default (because this happens when they are
							// holding nothing. Don't ask why.)
							click = ClickType.LEFT;
						...
						case DENY :
							...
							switch (action) {
								...
								case SWAP_WITH_CURSOR :
									...
									this.player.playerConnection.sendPacket(new PacketPlayOutSetSlot(this.player.activeContainer.windowId,
											packetplayinwindowclick.b(), this.player.activeContainer.getSlot(packetplayinwindowclick.b()).getItem()));
									break;
	...
	public void a(PacketPlayInEnchantItem packetplayinenchantitem) {
		...
		if (this.player.isFrozen())
			return; // CraftBukkit
		this.player.resetIdleTimer();
		if (this.player.activeContainer.windowId == packetplayinenchantitem.a() && this.player.activeContainer.c(this.player)
				...
				switch (event.getResult()) {
					...
					case DENY :
						...
						if (packetplayinsetcreativeslot.a() >= 0) {
							this.player.playerConnection
									.sendPacket(new PacketPlayOutSetSlot(this.player.defaultContainer.windowId, packetplayinsetcreativeslot.a(),
	...
	public void a(PacketPlayInTransaction packetplayintransaction) {
		...
		if (oshort != null && packetplayintransaction.b() == oshort.shortValue()
				&& this.player.activeContainer.windowId == packetplayintransaction.a() && !this.player.activeContainer.c(this.player)
				&& !this.player.isSpectator()) {
	...
	public void a(PacketPlayInUpdateSign packetplayinupdatesign) {
		...
		if (worldserver.isLoaded(blockposition)) {
			...
			SignChangeEvent event = new SignChangeEvent((org.bukkit.craftbukkit.block.CraftBlock) player.getWorld().getBlockAt(x, y, z),
	...
	public void a(PacketPlayInAbilities packetplayinabilities) {
		...
		if (this.player.abilities.canFly && this.player.abilities.isFlying != packetplayinabilities.isFlying()) {
			...
			this.server.getPluginManager().callEvent(event);
			if (!event.isCancelled()) {
				this.player.abilities.isFlying = packetplayinabilities.isFlying(); // Actually
																					// set
																					// the
																					// player's
																					// flying
				...
				this.player.updateAbilities(); // Tell the player their ability
												// was reverted
			}
		}
		// CraftBukkit end
			...
			if ("MC|BSign".equals(s)) {
				...
				if ("MC|AdvCmd".equals(s)) {
					...
					try {
						...
						if (b0 == 0) {
							tileentity = this.player.world.getTileEntity(new BlockPosition(packetdataserializer.readInt(),
					...
					try {
						...
						BlockPosition blockposition = new BlockPosition(packetdataserializer.readInt(), packetdataserializer.readInt(),
					...
					if ("MC|Beacon".equals(s)) {
						...
						try {
							BlockPosition blockposition1 = new BlockPosition(packetdataserializer.readInt(), packetdataserializer.readInt(),
									packetdataserializer.readInt());
							...
							if (tileentity instanceof TileEntityStructure) {
								...
								if (b1 == 2) {
									if (tileentitystructure.q()) {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.save_success", new Object[]{s7})),
												false);
									} else {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.save_failure", new Object[]{s7})),
												false);
									}
								} else if (b1 == 3) {
									if (!tileentitystructure.E()) {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.load_not_found", new Object[]{s7})),
												false);
									} else if (tileentitystructure.r()) {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.load_success", new Object[]{s7})),
												false);
									} else {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.load_prepare", new Object[]{s7})),
												false);
									}
								} else if (b1 == 4) {
									if (tileentitystructure.p()) {
										this.player.a((IChatBaseComponent) (new ChatMessage("structure_block.size_success", new Object[]{s7})),
												false);
									} else {
