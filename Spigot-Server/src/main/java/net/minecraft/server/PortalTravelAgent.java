...
public class PortalTravelAgent {
	...
	// Split out from original a(Entity, double, double, double, float) method
	// in order to enable being called from createPortal
	...
	public boolean b(Entity entity, float f) {
		// CraftBukkit start - Modularize portal search process and entity
		// teleportation
		...
		if (this.c.containsKey(k)) {
			...
			for (int l = -radius; l <= radius; ++l) {
				...
				for (int i1 = -radius; i1 <= radius; ++i1) {
					for (BlockPosition blockposition2 = blockposition.a(l, this.world.ab() - 1 - blockposition.getY(), i1); blockposition2
							.getY() >= 0; blockposition2 = blockposition1) {
						blockposition1 = blockposition2.down();
						if (this.world.getType(blockposition2).getBlock() == blocks.PORTAL) {
							for (blockposition1 = blockposition2.down(); this.world.getType(blockposition1)
									.getBlock() == blocks.PORTAL; blockposition1 = blockposition1.down()) {
	...
	// Entity repositioning logic split out from original b method and combined
	// with repositioning logic for The End from original a method
	public void adjustExit(Entity entity, Location position, Vector velocity) {
		...
		if (this.world.getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END
				|| entity.getBukkitEntity().getWorld().getEnvironment() == org.bukkit.World.Environment.THE_END || entity.getPortalOffset() == null) {
			// entity.setPositionRotation((double) i, (double) j, (double) k,
			// entity.yaw, 0.0F);
			// entity.motX = entity.motY = entity.motZ = 0.0D;
			...
			double d4 = shapedetector_shapedetectorcollection.getFacing().k() == EnumDirection.EnumAxis.X
					? (double) shapedetector_shapedetectorcollection.a().getZ()
					: (double) shapedetector_shapedetectorcollection.a().getX();
			double d5 = (double) (shapedetector_shapedetectorcollection.a().getY() + 1)
					- entity.getPortalOffset().y * (double) shapedetector_shapedetectorcollection.e();
			...
			if (shapedetector_shapedetectorcollection.getFacing().k() == EnumDirection.EnumAxis.X) {
				d3 = d4 + (1.0D - entity.getPortalOffset().x) * (double) shapedetector_shapedetectorcollection.d()
						* (double) shapedetector_shapedetectorcollection.getFacing().e().c().a();
			} else {
				d2 = d4 + (1.0D - entity.getPortalOffset().x) * (double) shapedetector_shapedetectorcollection.d()
						* (double) shapedetector_shapedetectorcollection.getFacing().e().c().a();
			}
			...
			// CraftBukkit start - Adjust position and velocity instances
			// instead of entity
			velocity.setX(d6 * (double) f1 + d7 * (double) f4);
			velocity.setZ(d6 * (double) f3 + d7 * (double) f2);
			f = f - (float) (entity.getPortalDirection().opposite().get2DRotationValue() * 90)
					+ (float) (shapedetector_shapedetectorcollection.getFacing().get2DRotationValue() * 90);
		...
		if (event.isCancelled() || to == null || !entity.isAlive()) {
			...
			velocity.copy(event.getAfter()); // event.getAfter() will never be
												// null, as setAfter() will
												// cause an NPE if null is
												// passed in
		}
		// CraftBukkit end
	}
	...
	public boolean a(Entity entity) {
		// CraftBukkit start - Allow for portal creation to be based on
		// coordinates instead of entity
	...
	public boolean createPortal(double x, double y, double z, int b0) {
		...
		for (i2 = i - 16; i2 <= i + 16; ++i2) {
			...
			for (j2 = k - 16; j2 <= k + 16; ++j2) {
				...
				label271 : for (k2 = this.world.ab() - 1; k2 >= 0; --k2) {
							...
							for (k3 = 0; k3 < 3; ++k3) {
								for (l3 = 0; l3 < 4; ++l3) {
									for (i4 = -1; i4 < 4; ++i4) {
										...
										if (i4 < 0 && !this.world.getType(blockposition_mutableblockposition).getMaterial().isBuildable()
		...
		if (d0 < 0.0D) {
			for (i2 = i - 16; i2 <= i + 16; ++i2) {
				...
				for (j2 = k - 16; j2 <= k + 16; ++j2) {
					...
					label219 : for (k2 = this.world.ab() - 1; k2 >= 0; --k2) {
						if (this.world.isEmpty(blockposition_mutableblockposition.c(i2, k2, j2))) {
							while (k2 > 0 && this.world.isEmpty(blockposition_mutableblockposition.c(i2, k2 - 1, j2))) {
								--k2;
							}
							...
							for (l2 = l1; l2 < l1 + 2; ++l2) {
								...
								for (k3 = 0; k3 < 4; ++k3) {
									for (l3 = -1; l3 < 4; ++l3) {
										...
										if (l3 < 0 && !this.world.getType(blockposition_mutableblockposition).getMaterial().isBuildable()
	...
	public void a(long i) {
		if (i % 100L == 0L) {
			...
			while (objectiterator.hasNext()) {
				PortalTravelAgent.ChunkCoordinatesPortal portaltravelagent_chunkcoordinatesportal = (PortalTravelAgent.ChunkCoordinatesPortal) objectiterator
						.next();
