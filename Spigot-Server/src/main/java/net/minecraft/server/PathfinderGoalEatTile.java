...
public class PathfinderGoalEatTile extends PathfinderGoal {
	...
	private static final Predicate<IBlockData> b = BlockStatePredicate.a((Block) blocks.TALLGRASS).a(BlockLongGrass.TYPE,
		...
		if (this.a == 4) {
			...
			if (PathfinderGoalEatTile.b.apply(this.d.getType(blockposition))) {
				...
				if (!CraftEventFactory.callEntityChangeBlockEvent(this.c,
						this.c.world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ()), Material.AIR,
						!this.d.getGameRules().getBoolean("mobGriefing")).isCancelled()) {
				...
				if (this.d.getType(blockposition1).getBlock() == blocks.GRASS) {
					...
					if (!CraftEventFactory.callEntityChangeBlockEvent(this.c,
							this.c.world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(), blockposition.getZ()), Material.AIR,
							!this.d.getGameRules().getBoolean("mobGriefing")).isCancelled()) {
						this.d.triggerEffect(2001, blockposition1, blockRegistry.getId(blocks.GRASS));
