...
public class NameReferencingFileConverter {
	...
	private void a(MinecraftServer minecraftserver, Collection<String> collection, ProfileLookupCallback profilelookupcallback) {
		...
		// TODON: Minecraftserver shouldn't store isOnlineMode it should come
		// right from the
		// Config
		if (minecraftserver.getOnlineMode() || cfg.isBungeecord()) { // Spigot:
																		// bungee
																		// =
																		// online
																		// mode,
	...
	public boolean a(final MinecraftServer minecraftserver) {
		...
		if (NameReferencingFileConverter.b.exists() && NameReferencingFileConverter.b.isFile()) {
			if (gameprofilebanlist.c().exists()) {
				try {
					...
					// CraftBukkit start - FileNotFoundException -> IOException,
					...
					public void onProfileLookupFailed(GameProfile gameprofile, Exception exception) {
						...
						if (!(exception instanceof ProfileNotFoundException)) {
							throw new NameReferencingFileConverter.FileConversionException(
									"Could not request user " + gameprofile.getName() + " from backend systems", exception, null);
						}
					}
				};
	...
	public boolean b(MinecraftServer minecraftserver) {
	...
	public boolean c(final MinecraftServer minecraftserver) {
	...
	public boolean d(final MinecraftServer minecraftserver) {
	...
	public String a(final MinecraftServer minecraftserver, String s) {
		if (!UtilColor.b(s) && s.length() <= 16) {
			...
			if (gameprofile != null && gameprofile.getId() != null) {
				...
				return !arraylist.isEmpty() && ((GameProfile) arraylist.get(0)).getId() != null
	...
	public boolean a(final DedicatedServer dedicatedserver, PropertyManager propertymanager) {
			...
			try {
				...
				ProfileLookupCallback profilelookupcallback = new ProfileLookupCallback() {
					...
					private void a(File file, String s, String s1) {
						...
						// CraftBukkit start - Use old file name to seed
						// lastKnownName
					...
					private String a(GameProfile gameprofile) {
						String s = null;
						// String[] astring = astring1; // CraftBukkit -
						// decompile error
						int i = astring.length;
	...
	private static boolean c(PropertyManager propertymanager) {
		...
		if (file.exists() && file.isDirectory() && (file.list().length > 0 || !file.delete())) {
			...
			NameReferencingFileConverter.e.warn("** please restart the server and if the problem persists, remove the directory \'{}\'",
					file.getPath());
			return false;
		} else {
			return true;
	...
	static class FileConversionException extends RuntimeException {
		...
		private static final long serialVersionUID = 7733252678886020730L;
