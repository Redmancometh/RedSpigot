...
public class EntityCreeper extends EntityMonster {
	...
	public void die(DamageSource damagesource) {
		...
		if (this.world.getGameRules().getBoolean("doMobLoot")) {
			if (damagesource.getEntity() instanceof EntitySkeleton) {
				int i = items.getId(Items.RECORD_13);
				int j = items.getId(Items.RECORD_WAIT);
				int k = i + this.random.nextInt(j - i + 1);

				this.a(items.getById(k), 1);
			} else if (damagesource.getEntity() instanceof EntityCreeper && damagesource.getEntity() != this
					&& ((EntityCreeper) damagesource.getEntity()).isPowered() && ((EntityCreeper) damagesource.getEntity()).canCauseHeadDrop()) {
	...
	public void onLightningStrike(EntityLightning entitylightning) {
		...
		// CraftBukkit start
		if (CraftEventFactory.callCreeperPowerEvent(this, entitylightning, org.bukkit.event.entity.CreeperPowerEvent.PowerCause.LIGHTNING)
				.isCancelled()) {
			return;
		}
