...
public final class ItemStack {
	...
	// Called to run this stack through the data converter to handle older
	// storage methods and serialized items
	public void convertStack() {
		if (MinecraftServer.getServer() != null) {
			// Don't convert beds - both the old and new data values are valid
			// Conversion would make getting white beds (data value 0)
			// impossible
			...
			MinecraftServer.getServer().dataConverterManager.a(DataConverterTypes.ITEM_INSTANCE, savedStack); // PAIL:
	...
	public void load(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKeyOfType("tag", 10)) {
			// CraftBukkit start - make defensive copy as this data may be
			// coming from the save thread
	...
	public EnumInteractionResult placeItem(EntityHuman entityhuman, World world, BlockPosition blockposition, EnumHand enumhand,
		...
		if (enuminteractionresult == EnumInteractionResult.SUCCESS && world.captureTreeGeneration && world.capturedBlockStates.size() > 0) {
			...
			if (event == null || !event.isCancelled()) {
				// Change the stack to its new contents if it hasn't been
				// tampered with.
		...
		if (enuminteractionresult == EnumInteractionResult.SUCCESS) {
			...
			world.capturedBlockStates.clear();
			if (blocks.size() > 1) {
				placeEvent = org.bukkit.craftbukkit.event.CraftEventFactory.callBlockMultiPlaceEvent(world, entityhuman, enumhand, blocks,
						blockposition.getX(), blockposition.getY(), blockposition.getZ());
			} else if (blocks.size() == 1) {
				placeEvent = org.bukkit.craftbukkit.event.CraftEventFactory.callBlockPlaceEvent(world, entityhuman, enumhand, blocks.get(0),
						blockposition.getX(), blockposition.getY(), blockposition.getZ());
			}

			if (placeEvent != null && (placeEvent.isCancelled() || !placeEvent.canBuild())) {
				enuminteractionresult = EnumInteractionResult.FAIL; // cancel
																	// placement
				...
				for (BlockState blockstate : blocks) {
					...
					if (!(block.getBlock() instanceof BlockTileEntity)) { // Containers
																			// get
																			// placed
																			// automatically
					...
					world.notifyAndUpdatePhysics(newblockposition, null, oldBlock.getBlockData(), block, updateFlag); // send
																														// null
																														// chunk
																														// as
																														// chunk.k()
				...
				// Special case juke boxes as they update their tile entity.
				...
				if (this.item == Items.SKULL) { // Special case skulls to allow
												// wither spawns to be cancelled
				...
				if (this.item instanceof ItemBlock) {
					...
					world.a(entityhuman, blockposition, soundeffecttype.e(), SoundCategory.BLOCKS, (soundeffecttype.a() + 1.0F) / 2.0F,
							soundeffecttype.b() * 0.8F);
				}
	...
	public NBTTagCompound save(NBTTagCompound nbttagcompound) {
		...
		if (this.tag != null) {
			nbttagcompound.set("tag", this.tag.clone()); // CraftBukkit - make
															// defensive copy,
															// data is going to
															// another thread
		}
	...
	public void setData(int i) {
		...
		// The crafting system uses this value for a special purpose so we have
		// to allow it
		if (i == 32767) {
			this.damage = i;
			return;
			...
			if (i > 0) {
				...
				if (entityplayer != null) {
					...
					org.bukkit.event.player.PlayerItemDamageEvent event = new org.bukkit.event.player.PlayerItemDamageEvent(
							entityplayer.getBukkitEntity(), item, i);
	...
	public static boolean equals(ItemStack itemstack, ItemStack itemstack1) {
		return itemstack.isEmpty() && itemstack1.isEmpty()
				...
				: (!itemstack.isEmpty() && !itemstack1.isEmpty()
						? (itemstack.tag == null && itemstack1.tag != null ? false : itemstack.tag == null || itemstack.tag.equals(itemstack1.tag))
						: false);
	}
	...
	private boolean d(ItemStack itemstack) {
		return this.count != itemstack.count
				? false
				: (this.getItem() != itemstack.getItem()
						? false
						: (this.damage != itemstack.damage
								? false
								: (this.tag == null && itemstack.tag != null ? false : this.tag == null || this.tag.equals(itemstack.tag))));
	}
		...
		if (this.hasTag() && this.tag.hasKeyOfType("AttributeModifiers", 9)) {
			...
			for (int i = 0; i < nbttaglist.size(); ++i) {
				...
				if (attributemodifier != null
						...
						&& attributemodifier.a().getLeastSignificantBits() != 0L && attributemodifier.a().getMostSignificantBits() != 0L) {
	...
	public void setItem(Item item) {
		this.item = item;
		this.setData(this.getData()); // CraftBukkit - Set data again to ensure
	...
	public IChatBaseComponent C() {
		...
		if (!this.g) {
			...
			ichatbasecomponent.getChatModifier()
					.setChatHoverable(new ChatHoverable(ChatHoverable.EnumHoverAction.SHOW_ITEM, new ChatComponentText(nbttagcompound.toString())));
