	...
	public void c(World world, BlockPosition blockposition) {
		if (world.worldProvider.m()) {
			...
			if (((Integer) iblockdata.get(BlockDaylightDetector.POWER)).intValue() != i) {
				i = org.bukkit.craftbukkit.event.CraftEventFactory.callRedstoneChange(world, blockposition.getX(), blockposition.getY(),
						blockposition.getZ(), ((Integer) iblockdata.get(POWER)), i).getNewCurrent(); // CraftBukkit
																										// -
																										// Call
																										// BlockRedstoneEvent
