...
public class EntityRabbit extends EntityAnimal {
	...
	protected void r() {
		...
		this.goalSelector.a(3, new PathfinderGoalTempt(this, 1.0D, items.getItemOf(blocks.YELLOW_FLOWER), false));
	...
	private boolean a(Item item) {
		return item == Items.CARROT || item == Items.GOLDEN_CARROT || item == items.getItemOf(blocks.YELLOW_FLOWER);
	}
	...
	protected void do_() {
		...
		this.world.addParticle(EnumParticle.BLOCK_DUST, this.locX + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width,
				this.locY + 0.5D + (double) (this.random.nextFloat() * this.length),
				this.locZ + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, 0.0D, 0.0D, 0.0D,
	...
	class PathfinderGoalEatCarrots extends PathfinderGoalGotoTarget {
		...
		public void e() {
			super.e();
			this.c.getControllerLook().a((double) this.b.getX() + 0.5D, (double) (this.b.getY() + 1), (double) this.b.getZ() + 0.5D, 10.0F,
					(float) this.c.N());
			if (this.f()) {
				...
				if (this.e && block instanceof BlockCarrots) {
					...
					if (integer.intValue() == 0) {
						// CraftBukkit start
						if (org.bukkit.craftbukkit.event.CraftEventFactory.callEntityChangeBlockEvent(this.c, blockposition, blocks.AIR, 0)
								.isCancelled()) {
							return;
						}
						// CraftBukkit end
						...
						if (org.bukkit.craftbukkit.event.CraftEventFactory
								.callEntityChangeBlockEvent(this.c, blockposition, block,
										block.toLegacyData(iblockdata.set(BlockCarrots.AGE, Integer.valueOf(integer.intValue() - 1))))
								.isCancelled()) {
							return;
						}
						// CraftBukkit end
						world.setTypeAndData(blockposition, iblockdata.set(BlockCarrots.AGE, Integer.valueOf(integer.intValue() - 1)), 2);
						world.triggerEffect(2001, blockposition, blockRegistry.getCombinedId(iblockdata));
					}
