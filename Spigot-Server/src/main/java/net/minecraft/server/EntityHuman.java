...
public abstract class EntityHuman extends EntityLiving {
	...
	protected InventoryEnderChest enderChest = new InventoryEnderChest(this); // CraftBukkit
																				// -
																				// add
																				// "this"
																				// to
	...
	protected FoodMetaData foodData = new FoodMetaData(this); // CraftBukkit -
																// add "this" to
																// constructor
	protected int bA;
	public float bB;
	...
	public EntityHuman(World world, GameProfile gameprofile) {
		...
		this.setPositionRotation((double) blockposition.getX() + 0.5D, (double) (blockposition.getY() + 1), (double) blockposition.getZ() + 0.5D,
				0.0F, 0.0F);
		this.ba = 180.0F;
	}
	...
	protected void cT() {
		...
		if (f != this.width || f1 != this.length) {
			...
			axisalignedbb = new AxisAlignedBB(axisalignedbb.a, axisalignedbb.b, axisalignedbb.c, axisalignedbb.a + (double) f,
					axisalignedbb.b + (double) f1, axisalignedbb.c + (double) f);
	...
	public void n() {
		...
		if (this.world.getDifficulty() == EnumDifficulty.PEACEFUL && this.world.getGameRules().getBoolean("naturalRegeneration")) {
			if (this.getHealth() < this.getMaxHealth() && this.ticksLived % 20 == 0) {
				// CraftBukkit - added regain reason of "REGEN" for filtering
				// purposes.
	...
	public EntityItem a(boolean flag) {
		...
		return this.a(this.inventory.splitStack(this.inventory.itemInHandIndex,
				flag && !this.inventory.getItemInHand().isEmpty() ? this.inventory.getItemInHand().getCount() : 1), false, true);
	}
	...
	public EntityItem a(ItemStack itemstack, boolean flag, boolean flag1) {
		if (itemstack.isEmpty()) {
			...
			if (event.isCancelled()) {
				...
				} else if (flag1 && cur.isSimilar(drop.getItemStack()) && cur.getAmount() < cur.getMaxStackSize()
		...
		dataconvertermanager.a(DataConverterTypes.PLAYER, new DataInspector() {
			public NBTTagCompound a(DataConverter dataconverter, NBTTagCompound nbttagcompound, int i) {
				...
				if (nbttagcompound.hasKeyOfType("ShoulderEntityLeft", 10)) {
					nbttagcompound.set("ShoulderEntityLeft",
							dataconverter.a(DataConverterTypes.ENTITY, nbttagcompound.getCompound("ShoulderEntityLeft"), i));
				}

				if (nbttagcompound.hasKeyOfType("ShoulderEntityRight", 10)) {
					nbttagcompound.set("ShoulderEntityRight",
							dataconverter.a(DataConverterTypes.ENTITY, nbttagcompound.getCompound("ShoulderEntityRight"), i));
				}
	...
	public void b(NBTTagCompound nbttagcompound) {
		...
		nbttagcompound.setString("SpawnWorld", spawnWorld); // CraftBukkit -
															// fixes bed spawns
															// for multiworld
															// worlds
	...
	// CraftBukkit start
	protected boolean damageEntity0(DamageSource damagesource, float f) { // void
																			// ->
																			// boolean
		if (true) {
				...
				if (f > 0.0F || f1 > 0.0F) {
					...
					if (entity instanceof EntityLiving) {
						...
						if (j > 0 && !entity.isBurning()) {
							...
							EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(this.getBukkitEntity(), entity.getBukkitEntity(),
									1);
					...
					if (flag5) {
						if (i > 0) {
							if (entity instanceof EntityLiving) {
								((EntityLiving) entity).a(this, (float) i * 0.5F, (double) MathHelper.sin(this.yaw * 0.017453292F),
										(double) (-MathHelper.cos(this.yaw * 0.017453292F)));
							} else {
								entity.f((double) (-MathHelper.sin(this.yaw * 0.017453292F) * (float) i * 0.5F), 0.1D,
										(double) (MathHelper.cos(this.yaw * 0.017453292F) * (float) i * 0.5F));
							}
							...
							while (iterator.hasNext()) {
								...
								if (entityliving != this && entityliving != entity && !this.r(entityliving) && this.h(entityliving) < 9.0D) {
									...
									if (entityliving.damageEntity(DamageSource.playerAttack(this).sweep(), f4)) {
										entityliving.a(this, 0.4F, (double) MathHelper.sin(this.yaw * 0.017453292F),
												(double) (-MathHelper.cos(this.yaw * 0.017453292F)));
									}
									// CraftBukkit end
								}
						...
						if (entity instanceof EntityLiving) {
							...
							if (this.world instanceof WorldServer && f5 > 2.0F) {
								...
								((WorldServer) this.world).a(EnumParticle.DAMAGE_INDICATOR, entity.locX,
										entity.locY + (double) (entity.length * 0.5F), entity.locZ, k, 0.1D, 0.0D, 0.1D, 0.2D, new int[0]);
							}
						}

						this.applyExhaustion(world.spigotConfig.combatExhaustion); // Spigot
																					// -
																					// Change
																					// to
																					// use
	...
	public void cX() {
		...
		if (this.world instanceof WorldServer) {
			((WorldServer) this.world).a(EnumParticle.SWEEP_ATTACK, this.locX + d0, this.locY + (double) this.length * 0.5D, this.locZ + d1, 0, d0,
					0.0D, d1, 0.0D, new int[0]);
		}
	...
	public EntityHuman.EnumBedResult a(BlockPosition blockposition) {
		...
		if (!this.world.isClientSide) {
			...
			List list = this.world.a(EntityMonster.class,
					new AxisAlignedBB((double) blockposition.getX() - 8.0D, (double) blockposition.getY() - 5.0D,
							(double) blockposition.getZ() - 8.0D, (double) blockposition.getX() + 8.0D, (double) blockposition.getY() + 5.0D,
							(double) blockposition.getZ() + 8.0D),
					(Predicate) (new EntityHuman.c(this, null)));
		...
		if (this.world.isLoaded(blockposition)) {
			...
			this.setPosition((double) ((float) blockposition.getX() + f), (double) ((float) blockposition.getY() + 0.6875F),
					(double) ((float) blockposition.getZ() + f1));
		} else {
			this.setPosition((double) ((float) blockposition.getX() + 0.5F), (double) ((float) blockposition.getY() + 0.6875F),
	...
	private boolean a(BlockPosition blockposition, EnumDirection enumdirection) {
		if (Math.abs(this.locX - (double) blockposition.getX()) <= 3.0D && Math.abs(this.locY - (double) blockposition.getY()) <= 2.0D
				&& Math.abs(this.locZ - (double) blockposition.getZ()) <= 3.0D) {
			return true;
		} else {
			BlockPosition blockposition1 = blockposition.shift(enumdirection.opposite());

			return Math.abs(this.locX - (double) blockposition1.getX()) <= 3.0D && Math.abs(this.locY - (double) blockposition1.getY()) <= 2.0D
					&& Math.abs(this.locZ - (double) blockposition1.getZ()) <= 3.0D;
		}
	}
	...
	public void a(boolean flag, boolean flag1, boolean flag2) {
		...
		if (this.bedPosition != null && iblockdata.getBlock() == blocks.BED) {
			...
			this.setPosition((double) ((float) blockposition.getX() + 0.5F), (double) ((float) blockposition.getY() + 0.1F),
					(double) ((float) blockposition.getZ() + 0.5F));
		}
	...
	public void cu() {
		...
		if (this.isSprinting()) {
			this.applyExhaustion(world.spigotConfig.jumpSprintExhaustion); // Spigot
																			// -
																			// Change
																			// to
																			// use
			...
			this.applyExhaustion(world.spigotConfig.jumpWalkExhaustion); // Spigot
																			// -
																			// Change
																			// to
																			// use
	...
	public ItemStack getEquipment(EnumItemSlot enumitemslot) {
		return enumitemslot == EnumItemSlot.MAINHAND
				...
				: (enumitemslot == EnumItemSlot.OFFHAND
						? (ItemStack) this.inventory.extraSlots.get(0)
						: (enumitemslot.a() == EnumItemSlot.Function.ARMOR ? (ItemStack) this.inventory.armor.get(enumitemslot.b()) : ItemStack.a));
	}
	...
	private boolean spawnEntityFromShoulder(@Nullable NBTTagCompound nbttagcompound) { // CraftBukkit
																						// void->boolean
	...
	public IChatBaseComponent getScoreboardDisplayName() {
		...
		chatcomponenttext.getChatModifier()
				.setChatClickable(new ChatClickable(ChatClickable.EnumClickAction.SUGGEST_COMMAND, "/msg " + this.getName() + " "));
	...
	static class c implements Predicate<EntityMonster> {
		...
		public boolean apply(@Nullable EntityMonster object) { // CraftBukkit -
																// decompile
																// error
