...
public class BlockVine extends Block {
	...
	public static final BlockStateBoolean[] f = new BlockStateBoolean[]{BlockVine.UP, BlockVine.NORTH, BlockVine.SOUTH, BlockVine.WEST,
			BlockVine.EAST};
	...
	public BlockVine() {
		...
		this.w(this.blockStateList.getBlockData().set(BlockVine.UP, Boolean.valueOf(false)).set(BlockVine.NORTH, Boolean.valueOf(false))
				.set(BlockVine.EAST, Boolean.valueOf(false)).set(BlockVine.SOUTH, Boolean.valueOf(false))
				.set(BlockVine.WEST, Boolean.valueOf(false)));
		this.a(true);
	...
	public IBlockData updateState(IBlockData iblockdata, IBlockAccess iblockaccess, BlockPosition blockposition) {
		...
		return iblockdata.set(BlockVine.UP, Boolean
				.valueOf(iblockaccess.getType(blockposition1).d(iblockaccess, blockposition1, EnumDirection.DOWN) == EnumBlockFaceShape.SOLID));
	}
	...
	public boolean a(World world, BlockPosition blockposition, EnumDirection enumdirection) {
		...
		return this.c(world, blockposition.shift(enumdirection.opposite()), enumdirection)
				&& (block == blocks.AIR || block == blocks.VINE || this.c(world, blockposition.up(), EnumDirection.UP));
	}
	...
	protected boolean e(Block block) {
		return block instanceof BlockShulkerBox || block == blocks.BEACON || block == blocks.cauldron || block == blocks.GLASS
				|| block == blocks.STAINED_GLASS || block == blocks.PISTON || block == blocks.STICKY_PISTON || block == blocks.PISTON_HEAD
		...
		if (!world.isClientSide) {
			if (world.random.nextInt(Math.max(1, (int) (100.0F / world.spigotConfig.vineModifier) * 4)) == 0) { // Spigot
				...
				if (enumdirection == EnumDirection.UP && blockposition.getY() < 255 && world.isEmpty(blockposition1)) {
					...
					if (((Boolean) iblockdata1.get(BlockVine.NORTH)).booleanValue() || ((Boolean) iblockdata1.get(BlockVine.EAST)).booleanValue()
					...
					if (enumdirection.k().c() && !((Boolean) iblockdata.get(getDirection(enumdirection))).booleanValue()) {
						if (!flag1) {
							...
							if (block.material == Material.AIR) {
								...
								// CraftBukkit start - Call BlockSpreadEvent
								org.bukkit.block.Block source = world.getWorld().getBlockAt(blockposition.getX(), blockposition.getY(),
										blockposition.getZ());
								org.bukkit.block.Block bukkitBlock = world.getWorld().getBlockAt(blockposition2.getX(), blockposition2.getY(),
										blockposition2.getZ());

								if (flag2 && this.a(world, blockposition3.shift(enumdirection2), enumdirection2)) {
									// world.setTypeAndData(blockposition2,
									// this.getBlockData().set(getDirection(enumdirection2),
									// Boolean.valueOf(true)), 2);
									CraftEventFactory.handleBlockSpreadEvent(bukkitBlock, source, this,
											toLegacyData(this.getBlockData().set(getDirection(enumdirection2), Boolean.valueOf(true))));
								} else if (flag3 && this.a(world, blockposition4.shift(enumdirection3), enumdirection3)) {
									// world.setTypeAndData(blockposition2,
									// this.getBlockData().set(getDirection(enumdirection3),
									// Boolean.valueOf(true)), 2);
									CraftEventFactory.handleBlockSpreadEvent(bukkitBlock, source, this,
											toLegacyData(this.getBlockData().set(getDirection(enumdirection3), Boolean.valueOf(true))));
								} else if (flag2 && world.isEmpty(blockposition3) && this.a(world, blockposition3, enumdirection)) {
									// world.setTypeAndData(blockposition3,
									// this.getBlockData().set(getDirection(enumdirection.opposite()),
									// Boolean.valueOf(true)), 2);
									bukkitBlock = world.getWorld().getBlockAt(blockposition3.getX(), blockposition3.getY(), blockposition3.getZ());
									CraftEventFactory.handleBlockSpreadEvent(bukkitBlock, source, this,
											toLegacyData(this.getBlockData().set(getDirection(enumdirection.opposite()), Boolean.valueOf(true))));
	...
	public IBlockData getPlacedState(World world, BlockPosition blockposition, EnumDirection enumdirection, float f, float f1, float f2, int i,
		...
		IBlockData iblockdata = this.getBlockData().set(BlockVine.UP, Boolean.valueOf(false)).set(BlockVine.NORTH, Boolean.valueOf(false))
				.set(BlockVine.EAST, Boolean.valueOf(false)).set(BlockVine.SOUTH, Boolean.valueOf(false)).set(BlockVine.WEST, Boolean.valueOf(false));
	...
	public void a(World world, EntityHuman entityhuman, BlockPosition blockposition, IBlockData iblockdata, @Nullable TileEntity tileentity,
		...
		if (!world.isClientSide && itemstack.getItem() == Items.SHEARS) {
			...
			// TODON: This will not work.
			a(world, blockposition, new ItemStack(blocks.VINE, 1, 0, null));
		} else {
	...
	public IBlockData fromLegacyData(int i) {
		return this.getBlockData().set(BlockVine.SOUTH, Boolean.valueOf((i & 1) > 0)).set(BlockVine.WEST, Boolean.valueOf((i & 2) > 0))
				.set(BlockVine.NORTH, Boolean.valueOf((i & 4) > 0)).set(BlockVine.EAST, Boolean.valueOf((i & 8) > 0));
	}
	...
	public IBlockData a(IBlockData iblockdata, EnumBlockRotation enumblockrotation) {
		switch (enumblockrotation) {
			case CLOCKWISE_180 :
				return iblockdata.set(BlockVine.NORTH, iblockdata.get(BlockVine.SOUTH)).set(BlockVine.EAST, iblockdata.get(BlockVine.WEST))
						.set(BlockVine.SOUTH, iblockdata.get(BlockVine.NORTH)).set(BlockVine.WEST, iblockdata.get(BlockVine.EAST));

			case COUNTERCLOCKWISE_90 :
				return iblockdata.set(BlockVine.NORTH, iblockdata.get(BlockVine.EAST)).set(BlockVine.EAST, iblockdata.get(BlockVine.SOUTH))
						.set(BlockVine.SOUTH, iblockdata.get(BlockVine.WEST)).set(BlockVine.WEST, iblockdata.get(BlockVine.NORTH));

			case CLOCKWISE_90 :
				return iblockdata.set(BlockVine.NORTH, iblockdata.get(BlockVine.WEST)).set(BlockVine.EAST, iblockdata.get(BlockVine.NORTH))
						.set(BlockVine.SOUTH, iblockdata.get(BlockVine.EAST)).set(BlockVine.WEST, iblockdata.get(BlockVine.SOUTH));
