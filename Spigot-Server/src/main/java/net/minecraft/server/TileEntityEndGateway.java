...
public class TileEntityEndGateway extends TileEntityEnderPortal implements ITickable {
		...
		if (!this.world.isClientSide && !this.f()) {
			...
			if (this.exitPortal != null) {
				...
				if (entity instanceof EntityPlayer) {
					...
					org.bukkit.Location location = new Location(world.getWorld(), (double) blockposition.getX() + 0.5D,
				...
				entity.enderTeleportTo((double) blockposition.getX() + 0.5D, (double) blockposition.getY() + 0.5D,
	...
	private BlockPosition a(World world, BlockPosition blockposition, int i, boolean flag) {
	...
	@Nullable
	private BlockPosition a(Chunk chunk) {
		...
		while (iterator.hasNext()) {
			...
			if (iblockdata.getBlock() == blocks.END_STONE && !chunk.getBlockData(blockposition3.up(1)).k()
					&& !chunk.getBlockData(blockposition3.up(2)).k()) {
