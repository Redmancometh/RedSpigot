...
public abstract class EntityHorseAbstract extends EntityAnimal implements IInventoryListener, IJumpable {
	...
	public static final IAttribute attributeJumpStrength = (new AttributeRanged((IAttribute) null, "horse.jumpStrength", 0.7D, 0.0D, 2.0D))
	...
	protected int bG;
	public int maxDomestication = 100; // CraftBukkit - store max domestication
										// value
	@Autowired
	private NameReferencingFileConverter names;
	...
	private void dl() {
		this.dp();
		if (!this.isSilent()) {
			this.world.a((EntityHuman) null, this.locX, this.locY, this.locZ, SoundEffects.cK, this.bK(), 1.0F,
	...
	public void e(float f, float f1) {
		...
		if (i > 0) {
			...
			if (iblockdata.getMaterial() != Material.AIR && !this.isSilent()) {
				...
				this.world.a((EntityHuman) null, this.locX, this.locY, this.locZ, soundeffecttype.d(), this.bK(), soundeffecttype.a() * 0.5F,
						soundeffecttype.b() * 0.75F);
			}
	...
	// TODON: This should not use direct field references...
	protected void initAttributes() {
		super.initAttributes();
		this.getAttributeMap().b(EntityHorseAbstract.attributeJumpStrength);
		this.getAttributeInstance(attr.maxHealth).setValue(53.0D);
		this.getAttributeInstance(attr.MOVEMENT_SPEED).setValue(0.22499999403953552D);
	}
	...
	public int getMaxDomestication() {
		return this.maxDomestication; // CraftBukkit - return stored max
										// domestication instead of 100
	}
	...
	protected boolean b(EntityHuman entityhuman, ItemStack itemstack) {
		...
		if (this.isBaby() && short0 > 0) {
			this.world.addParticle(EnumParticle.VILLAGER_HAPPY,
					...
					this.locZ + (double) (this.random.nextFloat() * this.width * 2.0F) - (double) this.width, 0.0D, 0.0D, 0.0D, new int[0]);
			if (!this.world.isClientSide) {
				this.setAge(short0);
			}
	...
	public void a(float f, float f1, float f2) {
		if (this.isVehicle() && this.cV() && this.dG()) {
			...
			if (this.bI()) {
				this.k((float) this.getAttributeInstance(attr.MOVEMENT_SPEED).getValue());
				super.a(f, f1, f2);
	...
	public void a(NBTTagCompound nbttagcompound) {
		...
		if (attributeinstance != null) {
			this.getAttributeInstance(attr.MOVEMENT_SPEED).setValue(attributeinstance.b() * 0.25D);
		}
	...
	protected boolean dL() {
		return !this.isVehicle() && !this.isPassenger() && this.isTamed() && !this.isBaby() && this.getHealth() >= this.getMaxHealth()
				&& this.isInLove();
	}
	...
	protected void a(EntityAgeable entityageable, EntityHorseAbstract entityhorseabstract) {
		double d0 = this.getAttributeInstance(attr.maxHealth).b() + entityageable.getAttributeInstance(attr.maxHealth).b() + (double) this.dM();

		entityhorseabstract.getAttributeInstance(attr.maxHealth).setValue(d0 / 3.0D);
		double d1 = this.getAttributeInstance(EntityHorseAbstract.attributeJumpStrength).b()
				+ entityageable.getAttributeInstance(EntityHorseAbstract.attributeJumpStrength).b() + this.dN();

		entityhorseabstract.getAttributeInstance(EntityHorseAbstract.attributeJumpStrength).setValue(d1 / 3.0D);
		double d2 = this.getAttributeInstance(attr.MOVEMENT_SPEED).b() + entityageable.getAttributeInstance(attr.MOVEMENT_SPEED).b() + this.dO();

		entityhorseabstract.getAttributeInstance(attr.MOVEMENT_SPEED).setValue(d2 / 3.0D);
	}
