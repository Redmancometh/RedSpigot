...
public final class SpawnerCreature {
		...
		if (!flag && !flag1) {
			...
			for (int k1 = 0; k1 < j; ++k1) {
				...
				if ((!enumcreaturetype.d() || flag1) && (enumcreaturetype.d() || flag) && (!enumcreaturetype.e() || flag2)) {
					...
					int l1 = limit * i / a; // CraftBukkit - use per-world
											// limits
					...
					if ((mobcnt = getEntityCount(worldserver, enumcreaturetype.a())) <= limit * i / 256) {
						...
						int moblimit = (limit * i / 256) - mobcnt + 1; // Spigot
																		// - up
																		// to 1
																		// more
																		// than
																		// limit
						label120 : while (iterator1.hasNext() && (moblimit > 0)) { // Spigot
																					// -
																					// while
																					// more
																					// allowed
									...
									while (true) {
										if (j4 < i4) {
											label113 : {
												...
												if (!worldserver.isPlayerNearby((double) f, (double) k3, (double) f1, 24.0D)
														&& blockposition.distanceSquared((double) f, (double) k3, (double) f1) >= 576.0D) {
													...
													if (worldserver.a(enumcreaturetype, biomebase_biomemeta,
														...
														try {
															entityinsentient = (EntityInsentient) biomebase_biomemeta.b
																	.getConstructor(new Class[]{World.class}).newInstance(new Object[]{worldserver});
														} catch (Exception exception) {
															exception.printStackTrace();
															return j1;
														}

														entityinsentient.setPositionRotation((double) f, (double) k3, (double) f1,
																worldserver.random.nextFloat() * 360.0F, 0.0F);
														if (entityinsentient.P() && entityinsentient.canSpawn()) {
															groupdataentity = entityinsentient
																	.prepare(worldserver.D(new BlockPosition(entityinsentient)), groupdataentity);
	...
	public static boolean a(IBlockData iblockdata) {
		return iblockdata.k()
				? false
				: (iblockdata.m() ? false : (iblockdata.getMaterial().isLiquid() ? false : !BlockMinecartTrackAbstract.i(iblockdata)));
	}

	public boolean a(EntityInsentient.EnumEntityPositionType entityinsentient_enumentitypositiontype, World world, BlockPosition blockposition) {
		if (!world.getWorldBorder().a(blockposition)) {
			...
			if (entityinsentient_enumentitypositiontype == EntityInsentient.EnumEntityPositionType.IN_WATER) {
				return iblockdata.getMaterial() == Material.WATER && world.getType(blockposition.down()).getMaterial() == Material.WATER
						&& !world.getType(blockposition.up()).l();
			} else {
				...
				for (int j2 = 0; j2 < i1; ++j2) {
					...
					for (int k2 = 0; !flag && k2 < 4; ++k2) {
						...
						if (a(EntityInsentient.EnumEntityPositionType.ON_GROUND, world, blockposition)) {
							...
							try {
								entityinsentient = (EntityInsentient) biomebase_biomemeta.b.getConstructor(new Class[]{World.class})
							...
							entityinsentient.setPositionRotation((double) ((float) j1 + 0.5F), (double) blockposition.getY(),
									(double) ((float) k1 + 0.5F), random.nextFloat() * 360.0F, 0.0F);
							// CraftBukkit start - Added a reason for spawning
							// this creature, moved
