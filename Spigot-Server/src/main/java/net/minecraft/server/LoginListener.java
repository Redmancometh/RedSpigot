		...
		if (s == null) {
			...
			if (this.server.aG() >= 0 && !this.networkManager.isLocal()) {
				this.networkManager.sendPacket(new PacketLoginOutSetCompression(this.server.aG()), new ChannelFutureListener() {
					...
					public void operationComplete(ChannelFuture future) throws Exception { // CraftBukkit
																							// -
																							// fix
																							// decompile
																							// error
			...
			if (entityplayer != null) {
				...
				this.l = this.server.getPlayerList().processLogin(this.i, s); // CraftBukkit
																				// -
																				// add
																				// player
																				// reference
		...
		if (!Arrays.equals(this.e, packetlogininencryptionbegin.b(privatekey))) {
			...
			(new Thread("User Authenticator #" + LoginListener.b.incrementAndGet()) {
				public void run() {
					...
					try {
						String s = (new BigInteger(MinecraftEncryption.a("", LoginListener.this.server.O().getPublic(), LoginListener.this.loginKey)))
								.toString(16);

						LoginListener.this.i = LoginListener.this.server.az().hasJoinedServer(new GameProfile((UUID) null, gameprofile.getName()), s,
								this.a());
				...
				private InetAddress a() {
					...
					return LoginListener.this.server.ac() && socketaddress instanceof InetSocketAddress
