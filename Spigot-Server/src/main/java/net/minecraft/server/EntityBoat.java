...
public class EntityBoat extends Entity {
	...
	private static final DataWatcherObject<Boolean>[] e = new DataWatcherObject[]{DataWatcher.a(EntityBoat.class, DataWatcherRegistry.h),
			DataWatcher.a(EntityBoat.class, DataWatcherRegistry.h)};
	...
	// CraftBukkit start
	// PAIL: Some of these haven't worked since a few updates, and since 1.9
	// they are less and less applicable.
			...
			if (damagesource instanceof EntityDamageSourceIndirect && damagesource.getEntity() != null && this.w(damagesource.getEntity())) {
				...
				if (flag || this.p() > 40.0F) {
					...
					if (destroyEvent.isCancelled()) {
						this.setDamage(40F); // Maximize damage so this doesn't
												// get triggered again right
												// away
						return true;
					}
					// CraftBukkit end
	...
	public void B_() {
		...
		for (int i = 0; i <= 1; ++i) {
			if (this.a(i)) {
				if (!this.isSilent() && (double) (this.f[i] % 6.2831855F) <= 0.7853981852531433D
						&& ((double) this.f[i] + 0.39269909262657166D) % 6.2831854820251465D >= 0.7853981852531433D) {
					...
					if (soundeffect != null) {
						...
						this.world.a((EntityHuman) null, this.locX + d0, this.locY, this.locZ + d1, soundeffect, this.bK(), 1.0F,
		...
		List list = this.world.getEntities(this, this.getBoundingBox().grow(0.20000000298023224D, -0.009999999776482582D, 0.20000000298023224D),
				IEntitySelector.a(this));
		...
		if (!list.isEmpty()) {
			...
			for (int j = 0; j < list.size(); ++j) {
				...
				if (!entity.w(this)) {
					if (flag && this.bF().size() < 2 && !entity.isPassenger() && entity.width < this.width && entity instanceof EntityLiving
							&& !(entity instanceof EntityWaterAnimal) && !(entity instanceof EntityHuman)) {
	...
	public float n() {
		AxisAlignedBB axisalignedbb = this.getBoundingBox();
		AxisAlignedBB axisalignedbb1 = new AxisAlignedBB(axisalignedbb.a, axisalignedbb.b - 0.001D, axisalignedbb.c, axisalignedbb.d, axisalignedbb.b,
				axisalignedbb.f);
			...
			for (int k1 = i; k1 < j; ++k1) {
				for (int l1 = k; l1 < l; ++l1) {
					for (int i2 = i1; i2 < j1; ++i2) {
						...
						if (iblockdata.getMaterial() == Material.WATER
								&& d0 < (double) BlockFluids.h(iblockdata, this.world, blockposition_pooledblockposition)) {
					...
					if (!this.world.isClientSide && !this.dead) {
						...
						if (!destroyEvent.isCancelled()) {
							...
							if (this.world.getGameRules().getBoolean("doEntityDrops")) {
								...
								for (i = 0; i < 3; ++i) {
									this.a(new ItemStack(items.getItemOf(blocks.PLANKS), 1, this.getType().b()), 0.0F);
								}
	...
	public static enum EnumBoatType {
		...
		OAK(BlockWood.EnumLogVariant.OAK.a(), "oak"), SPRUCE(BlockWood.EnumLogVariant.SPRUCE.a(), "spruce"), BIRCH(BlockWood.EnumLogVariant.BIRCH.a(),
				"birch"), JUNGLE(BlockWood.EnumLogVariant.JUNGLE.a(),
						"jungle"), ACACIA(BlockWood.EnumLogVariant.ACACIA.a(), "acacia"), DARK_OAK(BlockWood.EnumLogVariant.DARK_OAK.a(), "dark_oak");
