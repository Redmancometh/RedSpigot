...
public class WorldServer extends World implements IAsyncTaskHandler {
	...
	private final WorldServer.BlockActionDataList[] U = new WorldServer.BlockActionDataList[]{new WorldServer.BlockActionDataList(null),
	...
	public World b() {
		...
		// CraftBukkit start
		if (this.dimension != 0) { // SPIGOT-3899 multiple worlds of
									// advancements not supported
	...
	public void doTick() {
		...
		// CraftBukkit start - Only call spawner if we have players online and
		// the world allows for mobs or animals
		long time = this.worldData.getTime();
		if (this.getGameRules().getBoolean("doMobSpawning") && this.worldData.getType() != WorldType.DEBUG_ALL_BLOCK_STATES
				&& (this.allowMonsters || this.allowAnimals) && (this instanceof WorldServer && this.players.size() > 0)) {
			timings.mobSpawn.startTiming(); // Spigot
			this.spawnerCreature.a(this, this.allowMonsters && (this.ticksPerMonsterSpawns != 0 && time % this.ticksPerMonsterSpawns == 0L),
	...
	private void c() {
		...
		// CraftBukkit start
		// If we stop due to everyone sleeping we should reset the weather
		// duration to some other random value.
		// Not that everyone ever manages to get the whole server to sleep at
		// the same time....
				...
				if (flag && flag1 && this.random.nextInt(100000) == 0) {
					...
					if (this.isRainingAt(blockposition)) {
						...
						if (this.getGameRules().getBoolean("doMobSpawning")
								&& this.random.nextDouble() < (double) difficultydamagescaler.b() * 0.01D) {
							...
							this.strikeLightning(new EntityLightning(this, (double) blockposition.getX(), (double) blockposition.getY(),
					...
					for (int j1 = 0; j1 < i1; ++j1) {
						...
						if (chunksection != Chunk.a && chunksection.shouldTick()) {
							for (int k1 = 0; k1 < i; ++k1) {
								...
								if (block.isTicking()) {
									block.a((World) this, new BlockPosition(i2 + j, k2 + chunksection.getYPosition(), j2 + k), iblockdata,
											this.random);
								}
	...
	protected BlockPosition a(BlockPosition blockposition) {
		...
		AxisAlignedBB axisalignedbb = (new AxisAlignedBB(blockposition1,
				new BlockPosition(blockposition1.getX(), this.getHeight(), blockposition1.getZ()))).g(3.0D);
	...
	public boolean a(boolean flag) {
		if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
			...
			if (false) { // CraftBukkit
				...
				if (i > 65536) {
					// CraftBukkit start - If the server has too much to process
					// over time, try to alleviate that
					if (i > 20 * 65536) {
						i = i / 20;
					} else {
						i = 65536;
		...
		for (int i = 0; i < 2; ++i) {
			...
			while (iterator.hasNext()) {
				...
				if (blockposition.getX() >= structureboundingbox.a && blockposition.getX() < structureboundingbox.d
						&& blockposition.getZ() >= structureboundingbox.c && blockposition.getZ() < structureboundingbox.f) {
					if (flag) {
						if (i == 0) {
							// this.nextTickListHash.remove(nextticklistentry);
							// // CraftBukkit - removed
	...
	public List<TileEntity> getTileEntities(int i, int j, int k, int l, int i1, int j1) {
		...
		for (int chunkX = (i >> 4); chunkX <= ((l - 1) >> 4); chunkX++) {
			for (int chunkZ = (k >> 4); chunkZ <= ((j1 - 1) >> 4); chunkZ++) {
				...
				for (Object te : chunk.tileEntities.values()) {
					...
					if ((tileentity.position.getX() >= i) && (tileentity.position.getY() >= j) && (tileentity.position.getZ() >= k)
							&& (tileentity.position.getX() < l) && (tileentity.position.getY() < i1) && (tileentity.position.getZ() < j1)) {
		...
		/*
		 * for (int k1 = 0; k1 < this.tileEntityList.size(); ++k1) { TileEntity
		 * tileentity = (TileEntity) this.tileEntityList.get(k1); BlockPosition
		 * blockposition = tileentity.getPosition();
		 * 
		 * if (blockposition.getX() >= i && blockposition.getY() >= j &&
		 * blockposition.getZ() >= k && blockposition.getX() < l &&
		 * blockposition.getY() < i1 && blockposition.getZ() < j1) {
		 * arraylist.add(tileentity); } }
		 */
		// CraftBukkit end
			...
			if (this.generator != null) {
				...
				if (spawn != null) {
					if (spawn.getWorld() != ((WorldServer) this).getWorld()) {
						throw new IllegalStateException("Cannot set spawn point for " + this.worldData.getName() + " to be in another world ("
	...
	private boolean j(Entity entity) {
		if (entity.dead) {
			// WorldServer.a.warn("Tried to add entity {} but it was marked as
			// removed already", EntityTypes.a(entity)); // CraftBukkit
			return false;
		} else {
			...
			if (this.entitiesByUUID.containsKey(uuid)) {
				...
				if (this.f.contains(entity1)) {
					...
					if (!(entity instanceof EntityHuman)) {
						// WorldServer.a.warn("Keeping entity {} that already
						// exists with UUID {}", EntityTypes.a(entity1),
						// uuid.toString()); // CraftBukkit
						return false;
					}
	...
	public boolean strikeLightning(Entity entity) {
		...
		if (super.strikeLightning(entity)) {
			this.server.getPlayerList().sendPacketNearby((EntityHuman) null, entity.locX, entity.locY, entity.locZ, 512.0D, dimension,
					new PacketPlayOutSpawnEntityWeather(entity)); // CraftBukkit
																	// - Use
																	// dimension
			return true;
		} else {
	...
	public Explosion createExplosion(@Nullable Entity entity, double d0, double d1, double d2, float f, boolean flag, boolean flag1) {
		...
		/*
		 * Remove Explosion explosion = new Explosion(this, entity, d0, d1, d2,
		 * f, flag, flag1);
		 * 
		 * explosion.a(); explosion.a(false);
		 */
		// CraftBukkit end - TODO: Check if explosions are still properly
		// implemented
		if (!flag1) {
		...
		while (iterator.hasNext()) {
			...
			if (entityhuman.d(d0, d1, d2) < 4096.0D) {
				((EntityPlayer) entityhuman).playerConnection
						.sendPacket(new PacketPlayOutExplosion(d0, d1, d2, f, explosion.getBlocks(), (Vec3D) explosion.b().get(entityhuman)));
			}
		}
		...
		while (!this.U[this.V].isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				if (this.a(blockactiondata)) {
					// CraftBukkit - this.worldProvider.dimension ->
					// this.dimension
					this.server.getPlayerList().sendPacketNearby((EntityHuman) null, (double) blockactiondata.a().getX(),
							...
							new PacketPlayOutBlockAction(blockactiondata.a(), blockactiondata.d(), blockactiondata.b(), blockactiondata.c()));
				}
			}
	...
	private boolean a(BlockActionData blockactiondata) {
		...
		return iblockdata.getBlock() == blockactiondata.d()
				? iblockdata.a(this, blockactiondata.a(), blockactiondata.b(), blockactiondata.c())
				: false;
	}
	...
	protected void t() {
		...
		super.t();
		/*
		 * CraftBukkit start if (this.n != this.o) {
		 * this.server.getPlayerList().a((Packet) (new
		 * PacketPlayOutGameStateChange(7, this.o)),
		 * this.worldProvider.getDimensionManager().getDimensionID()); }
		 * 
		 * if (this.p != this.q) { this.server.getPlayerList().a((Packet) (new
		 * PacketPlayOutGameStateChange(8, this.q)),
		 * this.worldProvider.getDimensionManager().getDimensionID()); }
		 * 
		 * if (flag != this.isRaining()) { if (flag) {
		 * this.server.getPlayerList().sendAll(new
	...
	public void a(EnumParticle enumparticle, boolean flag, double d0, double d1, double d2, int i, double d3, double d4, double d5, double d6,
			int... aint) {
		// CraftBukkit - visibility api support
		sendParticles(null, enumparticle, flag, d0, d1, d2, i, d3, d4, d5, d6, aint);
	}

	public void sendParticles(EntityPlayer sender, EnumParticle enumparticle, boolean flag, double d0, double d1, double d2, int i, double d3,
			double d4, double d5, double d6, int... aint) {
		// CraftBukkit end
		PacketPlayOutWorldParticles packetplayoutworldparticles = new PacketPlayOutWorldParticles(enumparticle, flag, (float) d0, (float) d1,
				(float) d2, (float) d3, (float) d4, (float) d5, (float) d6, i, aint);
	...
	public void a(EntityPlayer entityplayer, EnumParticle enumparticle, boolean flag, double d0, double d1, double d2, int i, double d3, double d4,
