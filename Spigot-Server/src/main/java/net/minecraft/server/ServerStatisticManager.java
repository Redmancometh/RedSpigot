...
public class ServerStatisticManager extends StatisticManager {
	...
	public ServerStatisticManager(MinecraftServer minecraftserver, File file) {
		...
		for (String name : cfg.getStats().getForcedStats().keySet()) {
			StatisticWrapper wrapper = new StatisticWrapper();
			wrapper.a(cfg.getStats().getForcedStats().get(name));
	...
	public void b() {
		if (cfg.getStats().isDisableSaving())
			return; // Spigot
		try {
		...
		while (iterator.hasNext()) {
			...
			if (((StatisticWrapper) entry.getValue()).b() != null) {
				...
				try {
					...
					ServerStatisticManager.b.warn("Couldn\'t save statistic {}: error serializing progress", ((Statistic) entry.getKey()).d(),
							throwable);
				}
