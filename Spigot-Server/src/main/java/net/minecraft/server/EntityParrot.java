...
public class EntityParrot extends EntityPerchable implements EntityBird {
	...
	private static final Predicate<EntityInsentient> bH = new Predicate() {
		public boolean a(@Nullable EntityInsentient entityinsentient) {
			return entityinsentient != null && EntityParrot.bK.containsKey(EntityTypes.b.a(entityinsentient.getClass())); // CraftBukkit
																															// -
																															// decompile
																															// error
		}
	...
	protected void initAttributes() {
		...
		this.getAttributeMap().b(attr.e);
		this.getAttributeInstance(attr.maxHealth).setValue(6.0D);
		this.getAttributeInstance(attr.e).setValue(0.4000000059604645D);
		...
		if (!entity.isSilent() && world.random.nextInt(50) == 0) {
			...
			if (!list.isEmpty()) {
				...
				if (!entityinsentient.isSilent()) {
					SoundEffect soundeffect = g(EntityTypes.b.a(entityinsentient.getClass())); // CraftBukkit
																								// -
																								// decompile
																								// error
	...
	public boolean a(EntityHuman entityhuman, EnumHand enumhand) {
		...
		if (!this.isTamed() && EntityParrot.bJ.contains(itemstack.getItem())) {
			...
			if (!this.isSilent()) {
				this.world.a((EntityHuman) null, this.locX, this.locY, this.locZ, SoundEffects.eJ, this.bK(), 1.0F,
						1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
			}

			if (!this.world.isClientSide) {
				if (this.random.nextInt(10) == 0
						&& !org.bukkit.craftbukkit.event.CraftEventFactory.callEntityTameEvent(this, entityhuman).isCancelled()) { // CraftBukkit
					this.c(entityhuman);
					this.p(true);
