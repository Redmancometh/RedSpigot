...
public class CraftLivingEntity extends CraftEntity implements LivingEntity {
	...
	public void setMaxHealth(double amount) {
		...
		getHandle().getAttributeInstance(attr.maxHealth).setValue(amount);
	...
	public void resetMaxHealth() {
		setMaxHealth(getHandle().getAttributeInstance(attr.maxHealth).getAttribute().getDefault());
	}
	...
	public boolean addPotionEffect(PotionEffect effect, boolean force) {
		...
		getHandle().addEffect(new MobEffect(effectRegistry.getId(effect.getType().getId()), effect.getDuration(), effect.getAmplifier(),
	...
	public boolean hasPotionEffect(PotionEffectType type) {
		return getHandle().hasEffect(effectRegistry.getId(type.getId()));
	}

	@Override
	public PotionEffect getPotionEffect(PotionEffectType type) {
		// TODON: Replace the obfuscated method names in effectRegistry with the
		// correct ones
		MobEffect handle = getHandle().getEffect(effectRegistry.getId(type.getId()));
		return (handle == null)
				? null
				: new PotionEffect(PotionEffectType.getById(effectRegistry.a(handle.getMobEffect())), handle.getDuration(), handle.getAmplifier(),
						handle.isAmbient(), handle.isShowParticles());
	}

	public void removePotionEffect(PotionEffectType type) {
		getHandle().removeEffect(effectRegistry.getId(type.getId()));
	}

	public Collection<PotionEffect> getActivePotionEffects() {
		List<PotionEffect> effects = new ArrayList<PotionEffect>();
		for (MobEffect handle : getHandle().effects.values()) {
			effects.add(new PotionEffect(PotionEffectType.getById(effectRegistry.a(handle.getMobEffect())), handle.getDuration(),
	...
	public <T extends Projectile> T launchProjectile(Class<? extends T> projectile, Vector velocity) {
		...
		if (Snowball.class.isAssignableFrom(projectile)) {
			...
			if (TippedArrow.class.isAssignableFrom(projectile)) {
				...
				((EntityTippedArrow) launch).setType(potions.fromBukkit(new PotionData(PotionType.WATER, false, false)));
