...
public class CraftTippedArrow extends CraftArrow implements TippedArrow {
	...
	public boolean addCustomEffect(PotionEffect effect, boolean override) {
		...
		for (MobEffect mobEffect : getHandle().effects) {
			if (effectRegistry.a(mobEffect.getMobEffect()) == effectId) {
				existing = mobEffect;
			}
		}
		if (existing != null) {
		...
		getHandle().a(potions.fromBukkit(effect));
	...
	public boolean hasCustomEffect(PotionEffectType type) {
		for (MobEffect effect : getHandle().effects) {
			if (potions.equals(effect.getMobEffect(), type)) {
				return true;
			}
		}
		return false;
	...
	public boolean removeCustomEffect(PotionEffectType effect) {
		...
		Validate.isTrue(getBasePotionData().getType() != PotionType.UNCRAFTABLE || !getHandle().effects.isEmpty(),
				"Tipped Arrows must have at least 1 effect");
		getHandle().effects.remove(existing);
		getHandle().refreshEffects();
		return true;
	}
	...
	public void setBasePotionData(PotionData data) {
		...
		getHandle().setType(potions.fromBukkit(data));
	}

	@Override
	public PotionData getBasePotionData() {
		return potions.toBukkit(getHandle().getType());
	}
