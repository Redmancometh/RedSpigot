...
public class CraftBlock implements Block {
	...
	public boolean setTypeIdAndData(final int type, final byte data, final boolean applyPhysics) {
		...
		// SPIGOT-611: need to do this to prevent glitchiness. Easier to handle
		// this here (like /setblock) than to fix weirdness in tile entity
		// cleanup
		...
		if (applyPhysics) {
			...
			boolean success = chunk.getHandle().getWorld().setTypeAndData(position, blockData, 18); // NOTIFY
																									// |
																									// NO_OBSERVER
			if (success) {
	...
	private boolean itemCausesDrops(ItemStack item) {
		...
		net.minecraft.server.Item itemType = item != null ? items.getById(item.getTypeId()) : null;
		return block != null && (block.getBlockData().getMaterial().isAlwaysDestroyable()
				|| (itemType != null && itemType.canDestroySpecialBlock(block.getBlockData())));
	}

	public boolean breakNaturally() {
		// Order matters here, need to drop before setting to air so skulls can
		// get their data
		net.minecraft.server.Block block = this.getNMSBlock();
		byte data = getData();
	...
	public Collection<ItemStack> getDrops() {
		...
		if (block != blocks.AIR) {
			...
			for (int i = 0; i < count; ++i) {
				...
				if (item != Items.a) {
					// Skulls are special, their data is based on the tile
					// entity
					if (blocks.SKULL == block) {
						...
						// We don't want to drop cocoa blocks, we want to drop
						// cocoa beans.
