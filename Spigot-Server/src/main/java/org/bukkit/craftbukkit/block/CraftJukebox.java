...
public class CraftJukebox extends CraftBlockEntityState<TileEntityRecordPlayer> implements Jukebox {
	...
	public Material getPlaying() {
		...
		return magicNumbers.getMaterial(record.getItem());
	}

	@Override
	public void setPlaying(Material record) {
		if (record == null || magicNumbers.getItem(record) == null) {
			record = Material.AIR;
		}

		this.getSnapshot().setRecord(new ItemStack(magicNumbers.getItem(record), 1));
		if (record == Material.AIR) {
			setRawData((byte) 0);
		} else {
			setRawData((byte) 1);
