...
public class CraftStatistic {
	...
	public static net.minecraft.server.Statistic getMaterialStatistic(org.bukkit.Statistic stat, Material material) {
		try {
			if (stat == Statistic.MINE_BLOCK) {
				return StatisticList.a(CraftMagicNumbers.getBlock(material)); // PAIL:
																				// getMineBlockStatistic
			}
			if (stat == Statistic.CRAFT_ITEM) {
				return StatisticList.a(CraftMagicNumbers.getItem(material)); // PAIL:
																				// getCraftItemStatistic
			}
			if (stat == Statistic.USE_ITEM) {
				return StatisticList.b(CraftMagicNumbers.getItem(material)); // PAIL:
																				// getUseItemStatistic
			}
			if (stat == Statistic.BREAK_ITEM) {
				return StatisticList.c(CraftMagicNumbers.getItem(material)); // PAIL:
																				// getBreakItemStatistic
			}
			if (stat == Statistic.PICKUP) {
				return StatisticList.d(CraftMagicNumbers.getItem(material)); // PAIL:
																				// getPickupStatistic
			}
			if (stat == Statistic.DROP) {
				return StatisticList.e(CraftMagicNumbers.getItem(material)); // PAIL:
																				// getDropItemStatistic
			}
	...
	public Material getMaterialFromStatistic(net.minecraft.server.Statistic statistic) {
		String statisticString = statistic.name;
		String val = statisticString.substring(statisticString.lastIndexOf(".") + 1);
		Item item = (Item) items.get(new MinecraftKey(val));
		if (item != null) {
			return Material.getMaterial(items.getId(item));
		}
		Block block = (Block) blocks.get(new MinecraftKey(val));
		if (block != null) {
			return Material.getMaterial(blocks.getId(block));
		}
		try {
