...
public class CraftBlockProjectileSource implements BlockProjectileSource {
	...
	@Autowired
	private CraftPotionUtil craftPotionUtil;
	...
	public Block getBlock() {
		return dispenserBlock.getWorld().getWorld().getBlockAt(dispenserBlock.getPosition().getX(), dispenserBlock.getPosition().getY(),
	...
	public <T extends Projectile> T launchProjectile(Class<? extends T> projectile, Vector velocity) {
		...
		if (Snowball.class.isAssignableFrom(projectile)) {
			...
			if (LingeringPotion.class.isAssignableFrom(projectile)) {
				...
				launch = new EntityPotion(world, iposition.getX(), iposition.getY(), iposition.getZ(),
						CraftItemStack.asNMSCopy(new ItemStack(org.bukkit.Material.SPLASH_POTION, 1)));
			}
		...
		if (launch instanceof IProjectile) {
			...
			// Copied from DispenseBehaviorProjectile
			((IProjectile) launch).shoot((double) enumdirection.getAdjacentX(), (double) ((float) enumdirection.getAdjacentY() + 0.1F),
					(double) enumdirection.getAdjacentZ(), b, a);
		}
