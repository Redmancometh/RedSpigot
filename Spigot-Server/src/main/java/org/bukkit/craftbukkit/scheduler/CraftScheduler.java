 ...
 * utility method is provided for this, {@link #cancelTask(int)}</li>
...
public class CraftScheduler implements BukkitScheduler {
	...
	private final Executor executor = Executors
			.newCachedThreadPool(new com.google.common.util.concurrent.ThreadFactoryBuilder().setNameFormat("Craft Scheduler Thread - %1$d").build()); // Spigot
	...
	public BukkitTask runTask(Runnable runnable, long delay, long period) {
		return handle(new CraftTask(null, runnable, nextId(), period), delay);
	...
	public boolean isQueued(final int taskId) {
		...
		for (CraftTask task = head.getNext(); task != null; task = task.getNext()) {
			if (task.getTaskId() == taskId) {
				return task.getPeriod() >= CraftTask.NO_REPEATING; // The task
																	// will run
			}
		}
		CraftTask task = runners.get(taskId);
	...
	public List<BukkitWorker> getActiveWorkers() {
		...
		for (final CraftTask taskObj : runners.values()) {
			// Iterator will be a best-effort (may fail to grab very new values)
			...
			synchronized (task.getWorkers()) {
				// This will never have an issue with stale threads; it's
				// state-safe
	...
	public void mainThreadHeartbeat(final int currentTick) {
		...
		while (isReady(currentTick)) {
			...
			if (task.isSync()) {
				...
				// (async tasks must live with race-conditions if they attempt
				// to cancel between these few lines of code)
			}
