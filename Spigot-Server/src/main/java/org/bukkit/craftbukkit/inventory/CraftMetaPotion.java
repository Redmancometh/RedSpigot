...
class CraftMetaPotion extends CraftMetaItem implements PotionMeta {
	...
	// Having an initial "state" in ItemMeta seems bit dirty but the UNCRAFTABLE
	// potion type
	// is treated as the empty form of the meta because it represents an empty
	...
	CraftMetaPotion(NBTTagCompound tag) {
		...
		if (tag.hasKey(DEFAULT_POTION.NBT)) {
			type = potions.toBukkit(tag.getString(DEFAULT_POTION.NBT));
		}
	...
	CraftMetaPotion(Map<String, Object> map) {
		super(map);
		type = potions.toBukkit(SerializableMeta.getString(map, DEFAULT_POTION.BUKKIT, true));
	...
	void applyToItem(NBTTagCompound tag) {
		...
		tag.setString(DEFAULT_POTION.NBT, potions.fromBukkit(type));
	...
	Builder<String, Object> serialize(Builder<String, Object> builder) {
		...
		if (type.getType() != PotionType.UNCRAFTABLE) {
			builder.put(DEFAULT_POTION.BUKKIT, potions.fromBukkit(type));
		}
