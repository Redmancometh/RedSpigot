...
class CraftMetaItem implements ItemMeta, Repairable {
	@Autowired
	private GenericAttributes attr;
	@Autowired
	...
	public static class SerializableMeta implements ConfigurationSerializable {
		...
		static {
			classMap = ImmutableMap.<Class<? extends CraftMetaItem>, String>builder().put(CraftMetaBanner.class, "BANNER")
					.put(CraftMetaBlockState.class, "TILE_ENTITY").put(CraftMetaBook.class, "BOOK").put(CraftMetaBookSigned.class, "BOOK_SIGNED")
					.put(CraftMetaSkull.class, "SKULL").put(CraftMetaLeatherArmor.class, "LEATHER_ARMOR").put(CraftMetaMap.class, "MAP")
					.put(CraftMetaPotion.class, "POTION").put(CraftMetaSpawnEgg.class, "SPAWN_EGG").put(CraftMetaEnchantedBook.class, "ENCHANTED")
					.put(CraftMetaFirework.class, "FIREWORK").put(CraftMetaCharge.class, "FIREWORK_EFFECT")
					.put(CraftMetaKnowledgeBook.class, "KNOWLEDGE_BOOK").put(CraftMetaItem.class, "UNSPECIFIC").build();
	...
	CraftMetaItem(NBTTagCompound tag) {
		...
		if (tag.get(ATTRIBUTES.NBT) instanceof NBTTagList) {
			...
			attributesByName.put("generic.maxHealth", attr.maxHealth);
			attributeTracker.put("generic.followRange", 32.0);
			attributesByName.put("generic.followRange", attr.FOLLOW_RANGE);
			attributeTracker.put("generic.knockbackResistance", 0.0);
			attributesByName.put("generic.knockbackResistance", attr.c);
			attributeTracker.put("generic.movementSpeed", 0.7);
			attributesByName.put("generic.movementSpeed", attr.MOVEMENT_SPEED);
			attributeTracker.put("generic.attackDamage", 1.0);
			attributesByName.put("generic.attackDamage", attr.ATTACK_DAMAGE);
			...
			for (int i = 0; i < oldList.size(); ++i) {
				...
				if (!(nbttagcompound.get(ATTRIBUTES_IDENTIFIER.NBT) instanceof NBTTagString)
						|| !CraftItemFactory.KNOWN_NBT_ATTRIBUTE_NAMES.contains(nbttagcompound.getString(ATTRIBUTES_IDENTIFIER.NBT))) {
					continue;
				}
				...
				if (!nbttagcompound.hasKeyOfType(ATTRIBUTES_TYPE.NBT, 99) || nbttagcompound.getInt(ATTRIBUTES_TYPE.NBT) < 0
			...
			for (int i = 0; i < nbttaglist.size(); ++i) {
				...
				if (!nbttagcompound.hasKeyOfType(ATTRIBUTES_TYPE.NBT, CraftMagicNumbers.NBT.TAG_ANY_NUMBER)
						|| nbttagcompound.getInt(ATTRIBUTES_TYPE.NBT) < 0 || nbttagcompound.getInt(ATTRIBUTES_TYPE.NBT) > 2) {
					continue;
				}
		...
		if (hideFlags != null) {
			for (Object hideFlagObject : hideFlags) {
				...
				try {
					...
					// Ignore when we got a old String which does not map to a
					// Enum value anymore
				}
			}
		}
	...
	static void applyEnchantments(Map<Enchantment, Integer> enchantments, NBTTagCompound tag, ItemMetaKey key) {
		if (enchantments == null /* || enchantments.size() == 0 */) { // Spigot
																		// -
																		// remove
																		// size
																		// check
	...
	@Overridden
	boolean isEmpty() {
		return !(hasDisplayName() || hasLocalizedName() || hasEnchants() || hasLore() || hasRepairCost() || !unhandledTags.isEmpty() || hideFlag != 0
				|| isUnbreakable());
	}
	...
	public void setLore(List<String> lore) { // too tired to think if .clone is
												// better
		if (lore == null) {
			this.lore = null;
		} else {
	 ...
	 * common internals are unequal. Checking your own internals is redundant if
	 * you are not common, as notUncommon is meant for checking those 'not
	 * common' variables.
	 */
	@Overridden
	...
	/**
	 * This method is a bit weird... Return true if you are a common class OR
	 * your uncommon parts are empty. Empty uncommon parts implies the NBT data
	 * would be equivalent if both were applied to an item
	 */
	@Overridden
	...
	public final String toString() {
		return SerializableMeta.classMap.get(getClass()) + "_META:" + serialize(); // TODO:
																					// cry
	}
	...
	public static Set<String> getHandledTags() {
		synchronized (HANDLED_TAGS) {
			if (HANDLED_TAGS.isEmpty()) {
				HANDLED_TAGS
						.addAll(Arrays.asList(DISPLAY.NBT, REPAIR.NBT, ENCHANTMENTS.NBT, HIDEFLAGS.NBT, UNBREAKABLE.NBT, CraftMetaMap.MAP_SCALING.NBT,
								CraftMetaPotion.POTION_EFFECTS.NBT, CraftMetaPotion.DEFAULT_POTION.NBT, CraftMetaSkull.SKULL_OWNER.NBT,
								CraftMetaSkull.SKULL_PROFILE.NBT, CraftMetaSpawnEgg.ENTITY_TAG.NBT, CraftMetaBlockState.BLOCK_ENTITY_TAG.NBT,
								CraftMetaBook.BOOK_TITLE.NBT, CraftMetaBook.BOOK_AUTHOR.NBT, CraftMetaBook.BOOK_PAGES.NBT, CraftMetaBook.RESOLVED.NBT,
								CraftMetaBook.GENERATION.NBT, CraftMetaFirework.FIREWORKS.NBT, CraftMetaEnchantedBook.STORED_ENCHANTMENTS.NBT,
								CraftMetaCharge.EXPLOSION.NBT, CraftMetaBlockState.BLOCK_ENTITY_TAG.NBT, CraftMetaKnowledgeBook.BOOK_RECIPES.NBT));
			}
			return HANDLED_TAGS;
		}
	}
